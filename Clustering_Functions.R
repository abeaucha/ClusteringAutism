...
MakeModelList<-function(scanbase_scans_file="Data/Resources/scanbase_40um - Scans_22July19.csv",
                        scanbase_studies_file="Data/Resources/scanbase_40um - Studies_22Jul19.csv",
  ...
  wt_codes <- c("B6", "Wt", "WT_131", "WT_167", "Chd7++", "Chd7ff", "CON","Kctd13Wt_LatWt","WND","WNA","Snf2L_WT")
  ...
  write.csv(scans, file = glue("{base_directory}/scanbase_scans_autism_filtered_31Jan22.csv"))
  ...
  save(list = "model_list", file = glue("{base_directory}/model_list_31Jan22.RData"))
  return(model_list)
}
...
MakeEffectSizeMaps<-function(jdtype,model_list,resolution="200",
  ...
  scans<-read.csv(glue("{base_directory}/scanbase_scans_autism_filtered_31Jan22.csv"))
  ...
  scans<-read.csv(file = glue("{base_directory}/scanbase_scans_autism_filtered_31Jan22.csv"))
...
SNFCombineBoot<-function(K=10,alpha=0.5,T=20,distfunc="cor",
  ...
  mvol <- mincGetVolume(glue("{dir_determinants}/mask_{resolution}.mnc"))
  nvoxels <- length(which(mvol > 0.5))
...
GetNeighbourhoodEnrichment<-function(GeneScore="900",
  ...
  base_models_neighbourhood <- read.csv(glue("Data/Outputs/NeighbourhoodInfo/{GeneScore}/base_set_full_neighbourhood_{GeneScore}.txt"), header = F)$V1  %>% as.character
  ...
  for (num_clusters in 2:total_clusters) {
    ...
    for (clust in 1:num_clusters) {
      ...
      if (file.size(glue("Data/Outputs/NeighbourhoodInfo/{GeneScore}/target_set_cluster{num_clusters}_{clust}_{GeneScore}_neighbourhood.txt")) == 0) {
        next
      } else {
        target_set_cl <- read.csv(glue("Data/Outputs/NeighbourhoodInfo/{GeneScore}/target_set_cluster{num_clusters}_{clust}_{GeneScore}_neighbourhood.txt"), header = F)$V1 %>% as.character
        target$models_neighbourhood[[clust]] <- target_set_cl
        
        target_set_cl <- read.csv(glue("Data/Outputs/NeighbourhoodInfo/{GeneScore}/target_set_cluster{num_clusters}_{clust}_{GeneScore}_only_models.txt"), header = F)$V1 %>% as.character
...
GetSingleGeneNeighbourhoodEnrichment<-function(GeneScore="400",
  ...
  base_brain <- read.csv("Data/Raw/sagittal_gene_table_normalized_filtered.csv", header = T)$msg.genes.acronym %>% as.character
  base_models_neighbourhood <- read.csv(glue("Data/Outputs/NeighbourhoodInfo_150/{GeneScore}/base_set_full_neighbourhood_{GeneScore}.txt"), header = F)$V1  %>% as.character
  base_models_only <- all_df$gene
  base <- list(brain=base_brain, models_neighbourhood=base_models_neighbourhood, models_only=base_models_only)
  gmt_file<-Bader_List
  MappingsGMT <- tmodImportMSigDB(gmt_file, format = "gmt")
  ...
  for (SingleGene in 1:nrow(all_df)){
    ...
    if(file.size(file=glue("Data/Outputs/SingleGeneNeighbourhood_150/{GeneScore}/target_set_cluster_{sgene}_{GeneScore}_neighbourhood.txt"))==0){
      next
    }
    
    target_set <- read.csv(glue("Data/Outputs/SingleGeneNeighbourhood_150/{GeneScore}/target_set_cluster_{sgene}_{GeneScore}_neighbourhood.txt"), header = F)$V1 %>% as.character
