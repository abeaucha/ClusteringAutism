...
```{r}
col_selection <- str_detect(colnames(mat_sim_pos), str_c("^", nk, "-"))
row_selection <- str_detect(rownames(mat_sim_pos), str_c("^", nk, "-"))
mat_sim_pos_nk <- mat_sim_pos[row_selection, col_selection]  

col_selection <- str_detect(colnames(mat_sim_neg), str_c("^", nk, "-"))
row_selection <- str_detect(rownames(mat_sim_neg), str_c("^", nk, "-"))
mat_sim_neg_nk <- mat_sim_neg[row_selection, col_selection]  

mat_sim_nk_tmp <- array(data = 0,
                 dim = c(dim(mat_sim_pos_nk), 2))
mat_sim_nk_tmp[,,1] <- mat_sim_pos_nk
mat_sim_nk_tmp[,,2] <- mat_sim_neg_nk

mat_sim_nk_tmp <- rowMeans(mat_sim_nk_tmp, dims = 2, na.rm = TRUE)

colnames(mat_sim_nk_tmp) <- colnames(mat_sim_pos_nk)
rownames(mat_sim_nk_tmp) <- rownames(mat_sim_pos_nk)
...
i <- 1
species <- "mouse"
structs <- df_spokes[["mouse"]]
mask <- "positive"
remove <- FALSE
# cluster <- cluster_mask_neg
...
```{r fig.width = 10, fig.height = 10}
nk_list <- list(mouse = nk_mouse, human = 7)
k_list <- list(mouse = k_mouse, human = 7)
...
```{r fig.width = 10, fig.height = 10}
nk_list <- list(mouse = 5, human = 8)
k_list <- list(mouse = 2, human = 5)
