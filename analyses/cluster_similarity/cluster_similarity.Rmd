...
human_labels_file <- str_c("AHBA_microarray_labels_studyspace_", version, "_", human_resolution, "mm.mnc")
...
```{r}
tree <- Clone(human_tree)
mask <- human_mask
defs <- human_defs
labels <- human_labels
remove <- TRUE
imgdir <- human_cluster_map_dirs[[1]]
structs <- df_spokes[["human"]]
nk <- nk_human
k <- k_human
sign <- "negative"

human_atlas_reduced <- reduce_human_atlas(tree = human_tree,
                                          labels = labels,
                                          defs = defs,
                                          nodes = structs, 
                                          remove = TRUE)
...
```{r}
cluster_dirs <- c("human" = human_cluster_map_dirs[[1]], 
...
positive_fractions <- compute_cluster_fractions(cluster_dir = cluster_dirs[[s]],
                                                nk = nk[[s]],
                                                k = k[[s]],
                                                labels = atlas[["labels"]],
                                                defs = atlas[["defs"]],
...
negative_fractions <- compute_cluster_fractions(cluster_dir = cluster_dirs[[s]],
                                                nk = nk[[s]],
                                                k = k[[s]],
                                                labels = atlas[["labels"]],
                                                defs = atlas[["defs"]],
