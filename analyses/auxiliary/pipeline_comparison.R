...
registration_dir <- list(old = registration_dir_old, new = registration_dir_new)

pipeline_dir <- list(old = pipeline_dir_old, new = pipeline_dir_new)
...
masks <- map(.x = registration_dir, .f = function(x) {
  file.path(x, "reference_files", "mask_0.8mm.mnc")
})

es_dir_abs <- map(.x = es_dir, .f = function(x) {
  file.path(x, resolution, "absolute")
})
es_dir_rel <- map(.x = es_dir, .f = function(x) {
  file.path(x, resolution, "relative")
})

es_files_abs <- map(.x = es_dir_abs, .f = function(x) {
  sort(list.files(x, full.names = TRUE, pattern = "*.mnc"))
})
es_files_rel <- map(.x = es_dir_rel, .f = function(x) {
  sort(list.files(x, full.names = TRUE, pattern = "*.mnc"))
})
...
masks <- list(
  old = file.path(es_dir[["old"]], resolution, "mask_0.8mm_3.0mm.mnc"),
...
es_files_abs <- map(.x = es_dir_abs, .f = function(x) {
  file.path(x, "effect_sizes.csv")
})
es_matrices_abs <- map(.x = es_files_abs, .f = function(x) {
  as_tibble(data.table::fread(x, header = TRUE))
})
map(es_matrices_abs, dim)

es_matrices_abs <- map(.x = es_matrices_abs, .f = function(x) {
  x %>% select(-file)
})

es_matrices_abs_cor <- map2_dbl(
  .x = es_matrices_abs[["new"]],
...
es_matrices_rel <- map(.x = es_files_rel, .f = function(x) {
  as_tibble(data.table::fread(x, header = TRUE))
})
map(es_matrices_rel, dim)

es_matrices_rel <- map(.x = es_matrices_rel, .f = function(x) {
  x %>% select(-file)
})

es_matrices_rel_cor <- map2_dbl(
  .x = es_matrices_rel[["new"]],
...
affinity <- cluster_dir %>%
  map(.f = function(x) {
    file.path(x, resolution, "affinity.csv")
  }) %>%
  map(.f = read_csv, show_col_types = FALSE)

map(affinity, dim)

affinity_cor <- map2_dbl(
  .x = affinity[["new"]],
  .y = affinity[["old"]],
  .f = cor
)
...
clusters <- cluster_dir %>%
  map(.f = function(x) {
    file.path(x, resolution, "clusters.csv")
  }) %>%
  map(.f = read_csv, show_col_types = FALSE) %>%
  map(.f = function(x) {
    column_to_rownames(x, "ID")
  })
...
for (nk in nk_range) {
  clusters_nk <- map(clusters, .f = function(x) {
    x[, paste0("nk", nk)]
  })

  nk_grid <- expand_grid(k_old = 1:nk, k_new = 1:nk, overlap = 0)
  ...
  for (i in 1:nrow(nk_grid)) {
    ...
    nk_grid[[i, "overlap"]] <- length(intersect(files_k_old, files_k_new)) /
      length(union(files_k_old, files_k_new))
  }
...
centroid_dir_abs <- map(.x = centroid_dir, .f = function(x) {
  file.path(x, resolution, "absolute")
})
centroid_dir_rel <- map(.x = centroid_dir, .f = function(x) {
  file.path(x, resolution, "relative")
})

centroid_files_abs <- map(.x = centroid_dir_abs, .f = function(x) {
  sort(list.files(x, full.names = TRUE, pattern = "*.mnc"))
})
centroid_files_rel <- map(.x = centroid_dir_rel, .f = function(x) {
  sort(list.files(x, full.names = TRUE, pattern = "*.mnc"))
})
...
ti <- Sys.time()
centroid_abs_cor <- mclapply(
  X = centroid_files_abs_reorder,
  FUN = image_cor,
  masks = masks,
  mc.cores = 12
)
...
ti <- Sys.time()
centroid_rel_cor <- mclapply(
  X = centroid_files_rel_reorder,
  FUN = image_cor,
  masks = masks,
  mc.cores = 12
)
