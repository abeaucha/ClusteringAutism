...
simulate_clusters <- function(i, grid, nobs, outdir = "./", run_umap = FALSE) {
  ...
  # UMAP projection
  if (run_umap) {
    umap_out <- umap(x, n_components = 2, random_state = seed)
  }
  ...
  if (run_umap) {
    ...
    pdf(file = outfile_umap, width = unit(8, "inch"), height = unit(8, "inch"))
    print(p_umap)
    dev.off()
  }
