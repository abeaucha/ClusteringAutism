...
for (j in jacobians) {
  ...
  human_umap <- umap(df_human_es_data, n_components = 2, random_state = umap_seed)
  ...
  df_human_umap_long <- df_human_umap %>% 
    ...
    pivot_longer(cols = c(-x1, -x2), names_to = "nk", values_to = "k") %>% 
    mutate(nk = str_remove(nk, "nk"),
           nk = as.numeric(nk),
           nk = factor(nk),
           k = factor(k))
  
  list_human_umap[[j]] <- df_human_umap_long
  
}
```

```{r}
p_human_umap_abs <- ggplot(list_human_umap[["absolute"]], 
       aes(x = x1, y = x2, col = k)) + 
  geom_point() + 
  facet_wrap(~nk, nrow = 3) + 
  labs(title = "Human absolute effect size UMAP projection") + 
  theme_bw()

outfile <- paste0("human_umap_", umap_seed, "_absolute.pdf")
outfile <- file.path(output_dir, outfile)
pdf(file = outfile, 
    width = 12,
    height = 10)
print(p_human_umap_abs)
dev.off()
```

```{r}
p_human_umap_rel <- ggplot(list_human_umap[["relative"]], 
       aes(x = x1, y = x2, col = k)) + 
  geom_point() + 
  facet_wrap(~nk, nrow = 3) + 
  labs(title = "Human relative effect size UMAP projection") + 
  theme_bw()

outfile <- paste0("human_umap_", umap_seed, "_relative.pdf")
outfile <- file.path(output_dir, outfile)
pdf(file = outfile, 
    width = 12,
    height = 10)
print(p_human_umap_rel)
dev.off()
```
...
```{r}
df_human_cluster_counts <- df_human_clusters %>% 
  select(k = paste0("nk", nk_human)) %>% 
  group_by(k) %>% 
  count()

ggplot(df_human_cluster_counts, 
       aes(x = factor(k), y = n)) +
...
df_mouse_cluster_counts <- df_mouse_clusters %>% 
  select(k = paste0("nk", nk_mouse)) %>% 
  group_by(k) %>% 
  count()

ggplot(df_mouse_cluster_counts, 
       aes(x = factor(k), y = n)) +
  geom_col(fill = "grey70", col = "grey20") + 
  scale_y_continuous(breaks = seq(0, 1000, by = 40)) + 
  labs(x = "Cluster",
       y = "Count",
       title = "Number of patients in mouse clusters") +
  theme_bw()
```
