...
subtitle: "Part 1: Using sparse human expression data"
format: 
  html:
...
In this document I explore the possibility of evaluating the similarity between mouse and human neuroanatomical clusters of NDDs by first decomposing each cluster into a common gene expression space and then computing the similarity in that space. 

# Importing the cluster maps

We begin by importing the **relative effect size representative cluster maps** for all mouse and human clusters.
...
The voxel-wise cluster maps are stored in the columns of a matrix. Here is an example for the mouse clusters: 
...
Before decomposing these maps into gene space, we have to do a bit of wrangling on the human data. The human cluster maps are full voxel-wise images, with values for 1,442,744 voxels. However, only some of the voxels in these maps have associated expression data since the human expression data set only contains 3,702 unique samples. Additionally, some of those samples were taken at the same location, further reducing the number of voxels that have expression data. We want to extract only those voxels.
...
#Number of voxels with microarray data
print(nrow(mat_human_clusters_microarray))
```
...
Now that the cluster map images are aligned with the expression data in each species, we can decompose the maps in terms of the expression of each gene space variable. In this analysis, we will be using the latent space variables for the latent space generated using random seed 1.
...
A simple way to decompose each cluster map into the gene expression latent space is to compute the correlation of every cluster map with every latent space variable. For example, let's consider the mouse cluster 2-1. How does this cluster map correlate with the maps of different latent space variables?
...
human_gene_decomposition <- buildSimilarityMatrix(x1 = human_expr,
                                                  x2 = mat_human_clusters_microarray,
                                                  method = "correlation")
```

We can take a look at the distribution of clusters in the two-dimensional sub-space spanned by the variables $h_1$ and $h_3$.
...
It looks like something weird is happening, with human clusters returning very small correlation values to these latent space variables. What do the distributions of correlations look like across all variables? Is the case for all variables? 
...
It looks like the small correlation values aren't just an artefact of $h_1$ and $h_3$. The correlations between all human clusters and all latent space variables are basically null. 

Here are the scatter plots for the first three latent space variables: 
...
The human cluster maps seem to be uncorrelated with any of the latent space variables. There's the possibility that this might be due to the small number of microarray samples. Let's ignore this for now and move on to illustrate how the rest of this analysis would look.
...
A number of clusters exhibit relatively high correlations. In particular we see that mouse 5-5 is highly correlated to human 5-2. Note that these correspondences are different than what we found using the cluster mask averaging approach. 

We can examine the spatial maps of these clusters in the usual way: 

```{r}
#| code-summary: Mouse cluster map
...
```{r}
#| code-summary: Human cluster map
