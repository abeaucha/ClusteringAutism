...
betas <- map_dbl(.x = 1:ncol(mat_es_abs), .f = get_stats, x = mat_es_abs, stat = "beta")
tvals <- map_dbl(.x = 1:ncol(mat_es_abs), .f = get_stats, x = mat_es_abs, stat = "t")

df_stats <- tibble(beta = betas,
                   tval = tvals) %>% 
  mutate(pvals = 2*(1 - pt(q = abs(tvals), df = nrow(mat_es_abs))),
