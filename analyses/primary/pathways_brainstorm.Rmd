...
df_enrichment_all <- df_enrichment_all %>% 
  mutate(Title = ifelse(Title == "Signaling Pathways", "Signal Transduction", Title))
```
...
```{r}
root_excl <- c("Circadian clock", "Disease",
               "Digestion and absorption", "DNA Repair", "DNA Replication", 
               "Drug ADME", "Muscle contraction", "Protein localization",
               "Reproduction", "Sensory Perception", "Transport of small molecules")

root_w_lvl_1 <- c("Chromatin organization", "Gene expression (Transcription)", 
                  "Immune System", "Neuronal System", "Signal Transduction")

df_enrichment_lvl_0 <- df_enrichment_all %>%
  filter(level == 0) %>% 
  filter(!(Root %in% root_excl)) 

df_enrichment_lvl_1 <- df_enrichment_all %>% 
  filter(Root %in% root_w_lvl_1, 
         level == 1)
...
pathway_roots <- df_enrichment_lvl_0 %>% 
  pull(Root) %>% 
  unique() %>% 
  sort()
...
df_enrichment_lvl_0_1 <- bind_rows(df_enrichment_lvl_0, df_enrichment_lvl_1)

df_enrichment_lvl_0_1 <- df_enrichment_lvl_0_1 %>% 
  filter(Title %in% pathways_enriched)

list_pathways_lvl_1 <- vector(mode = "list", length = length(pathway_roots))
names(list_pathways_lvl_1) <- pathway_roots
for (i in 1:length(list_pathways_lvl_1)) {
  lvls <- df_enrichment_lvl_0_1 %>% 
    filter(Root == pathway_roots[i], 
           level == 1) %>% 
    pull(Title) %>% 
    unique() %>% 
...
df_enrichment_plot <- df_enrichment_lvl_0_1 %>% 
...
```{r}
df_enrichment_plot %>% pull(Title) %>% unique() %>% length()
```
...
```{r}
df_enrichment_nk10 <- df_enrichment_all %>% 
  filter(nk == 10) %>% 
  mutate(Title_trunc = str_trunc(Title, width = 40))


for (i in 1:length(pathway_roots)) {
  root <- pathway_roots[i]
  
  df_enrichment_nk10_i <- df_enrichment_nk10 %>% 
    filter(Root == root, 
           level < 5)
  
  pathways_enriched <- df_enrichment_nk10_i %>% 
    filter(adj.P.Val < 0.01) %>% 
    pull(Title) %>% 
    unique()
  
  df_enrichment_nk10_i <- df_enrichment_nk10_i %>% 
    filter(Title %in% pathways_enriched)
  
  NLQ_max <- df_enrichment_nk10_i %>% 
    pull(NLQ) %>% 
    max()
  
  plt <- ggplot(df_enrichment_nk10_i, aes(x = factor(k), y = Title_trunc, fill = NLQ)) + 
    geom_tile(col = "grey60") +
    ...
    scale_fill_gradientn(colors = heatmap_palette,
                         ...
                         na.value = "grey85") +
    # scale_fill_brewer(palette = "PuBu", na.value = "grey85") + 
    labs(x = "Mouse cluster", y = "Pathway", fill = "Enrichment",
         title = root) + 
    theme_bw() +
  ...
  outfile <- "pathway_enrichment_"
  outfile <- paste0(outfile, root_output)
...
```{r}
df_pathway_levels <- readxl::read_excel("pathway_levels.xlsx")
```
...
list_pathways <- vector(mode = "list", length = nrow(df_pathway_levels))
for (i in 1:length(list_pathways)) {
  root <- df_pathway_levels[[i, "Root"]]
  lvl <- df_pathway_levels[[i, "Level"]]
  list_pathways[[i]] <- df_enrichment_all %>% 
    filter(Root == root, level == lvl)
}

df_pathways <- bind_rows(list_pathways) %>% 
  mutate(Title_trunc = str_trunc(Title, width = 40))

pathways_enriched <- df_pathways %>% 
  filter(adj.P.Val < 0.01) %>% 
  pull(Title) %>% 
  unique()
...
plt <- ggplot(df_pathways, aes(x = factor(k), y = fct_rev(Title_trunc), fill = NLQ)) + 
  geom_tile(col = "grey60") +
  facet_grid(Root~nk,  scales = "free", space = "free", switch = "y") + 
  scale_x_discrete(expand = expansion()) + 
  scale_y_discrete(expand = expansion()) + 
...
outfile <- "figure2_new_draft_lvl_best.pdf"
outfile <- file.path(output_dir, outfile)
pdf(file = outfile, 
    width = unit(10, "in"),
    height = unit(11, "in"))
