...
for (i in 1:length(list_polar_plots)) {
  ...
  E_breaks_end <- E_max - E_max %% breaks_step + breaks_step
  q_breaks_end <- q_max - q_max %% breaks_step + breaks_step
  
  E_polar_breaks <- seq(breaks_start, E_breaks_end, by = breaks_step)
  q_polar_breaks <- seq(breaks_start, q_breaks_end, by = breaks_step)
  ...
  E_polar_plot <- ggplot(list_polar_plots[[i]], 
                         aes(x = pathway, 
                             ymin = 0, ymax = E, 
                             group = cluster, 
                             fill = cluster, 
  ...
  q_polar_plot <- ggplot(list_polar_plots[[i]], 
                         aes(x = pathway, 
