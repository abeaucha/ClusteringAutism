...
option_list <- list(
  ...
  make_option(
    ...
    type = 'character',
    help = paste(
      "Path to CSV file containing voxel- or sample-wise",
  ...
  make_option('--treelabels', type = 'character'),
  make_option('--nlabels', type = 'numeric'),
  make_option(
    '--greymatter',
    type = 'character',
    default = 'true',
    help = paste(
      "Option to use only grey matter measurements.",
      "[default %default]"
    )
  ),
  make_option('--labels', type = 'character'),
...
if (!(args[['greymatter']] %in% c('true', 'false'))) {
  stop(paste(
    "--greymatter must be one of [true, false].",
    "Got:",
    args[['greymatter']]
  ))
}
...
if (!(args[['verbose']] %in% c('true', 'false'))) {
  stop(paste(
    "--verbose must be one of [true, false].",
    "Got:",
    args[['verbose']]
  ))
}
...
if (!(species %in% c('mouse', 'human'))) {
  stop(paste("--species must be one of [mouse, human].", "Got:", species))
}
...
if (verbose) {
  message("Labelling data from file: ", fileMat)
}
...
dfExpr <- suppressMessages(data.table::fread(fileMat, header = TRUE)) %>%
  as_tibble()
...
if (verbose) {
  message("Importing the data tree...")
}
...
if (verbose) {
  message("Assigning labels to the expression matrix...")
}
...
if (region %in% names(listLabels)) {
  ...
  dfExpr[, 'Region'] <- labelRegions(
    measurements = rownames(dfExpr),
    tree = treePruned,
    treefield = treefield
  )
