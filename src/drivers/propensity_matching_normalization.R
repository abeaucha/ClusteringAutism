...
get_matched_controls <- function(
  ...
  if (ncontrols_test < ncontrols) {
    controls_tmp <- controls %>%
      filter(Sex == participant_sex, Site == participant_site)
...
executor <- function(
  ...
  controls_matched <- get_matched_controls(
  ...
  mincWriteVolume(effect_size, output.filename = outfile, clobber = TRUE)
...
demographics <- demographics %>%
  filter(!is.na(DX), !is.na(Age), !is.na(Sex), !is.na(Site), !is.na(Scanner))
