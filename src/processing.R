...
import_images <- function(
  ...
  if (!(output_format %in% format_opts)) {
    format_err <- str_c(
      "Argument output_format must be one of (",
  ...
  if (nproc > 1) {
    ...
    imgs <- foreach(i = 1:length(imgfiles), .packages = "RMINC") %do%
      {
        progress(n = i)
        import_image(
          img = imgfiles[i],
...
build_voxel_matrix <- function(
  imgfiles,
  mask = NULL,
  file_col = FALSE,
  sort = FALSE,
  save = FALSE,
  outfile = "voxel_matrix.csv",
  version = "v1",
  nproc = 1
) {
  # Import images as tibble
  df_imgs <- import_images(
    imgfiles = imgfiles,
    mask = mask,
  ...
  # Sort if desired
  if (sort) {
    df_imgs <- arrange(df_imgs, file)
  }

  # Remove input file column if desired
  if (!file_col) {
    df_imgs <- select(df_imgs, -file)
  }

  # Save data frame to file if desired
  if (save) {
    data.table::fwrite(x = df_imgs, file = outfile)
  }
...
vector_to_image <- function(x, outfile, mask, version = "v1") {
  ...
  if (length(x) != sum(ind_mask)) {
    stop(paste(
      "Number of elements in x does not match the number of non-zero",
      "voxels in the mask."
    ))
  }
...
matrix_to_images <- function(
  ...
  if (length(x) != length(outfiles)) {
    stop(
      "Number of entries in x along margin ",
      margin,
  ...
  if (nproc > 1) {
    ...
    out <- clusterMap(
      ...
      x,
      outfiles,
      MoreArgs = list(mask = mask, version = version)
    )
    stopCluster(cl)
  } else {
    out <- mapply(
...
threshold_intensity <- function(
  ...
  if (symmetric) {
    if (comparison == "gt") {
      ...
      stop(paste(
        "Argument comparison must be one of ['gt', 'lt']:",
        comparison
      ))
    }
  } else {
...
threshold_top_n <- function(img, n = 0.2, symmetric = TRUE, tolerance = 1e-5) {
  ...
  if (symmetric & (n < 0)) {
    stop(paste(
      "Setting n < 0 while symmetric = True",
  ...
  if (n > 0) {
    ...
    if (n < 1) {
      n <- as.integer(floor(n * length(sorted_values)))
    }
    top_n_index <- sorted_index[
      length(sorted_index):(length(sorted_index) - n + 1)
    ]
  } else if (n < 0) {
...
threshold_image <- function(
  ...
  if (method == "intensity") {
    ...
    img <- threshold_top_n(img = img, n = threshold, symmetric = symmetric)
  } else {
    stop(paste("Argument method must be one of ", "['intensity', 'top_n']"))
  }
...
normative_growth_norm <- function(
  imgdir,
  demographics,
  mask,
  outdir,
  ...
  if (verbose) {
    message("Importing demographics information...")
  }
  ...
  demographics <- demographics %>%
    filter(!is.na(DX), !is.na(Age), !is.na(Sex))
  ...
  if (!is.null(batch)) {
    ...
    if (!all(batch_check)) {
      stop(
        "Batch columns not found in demographics:\n",
  ...
  if (verbose) {
    message("Matching image files to demographics...")
  }
  ...
  if (verbose) {
    message("Evaluating normative growth models...")
  }
  if (execution == "local") {
    voxels <- mcMincApply(
      filenames = imgfiles,
    ...
    voxels <- qMincApply(
      ...
      registry_name = ifelse(
        is.null(registry_name),
  ...
  # Export images
  if (verbose) {
    message("Exporting normalized images...")
  }
...
#'
#' @return (character vector) Paths to the effect size images.
propensity_matching_norm <- function(
  imgdir,
  demographics,
  mask,
  outdir,
...
similarity_network <- function(
  ...
  if (metric == "correlation") {
    ...
    stop(paste(
      "Argument metric must be one of {correlation, euclidean}:",
      metric
    ))
  }
...
compute_cluster_centroids <- function(
  ...
  for (k in krange) {
    ...
    mincWriteVolume(centroid, output.filename = outfile, clobber = TRUE)
