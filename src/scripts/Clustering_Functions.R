...
MakeModelList <- function(
  scanbase_scans_file = "Data/Resources/scanbase_40um - Scans_22July19.csv",
  ...
  # Fix Pten_Kim study
  scans$Study_Name[which(
    scans$Study_Name == "PTEN_Kim" & endsWith(scans$Genotype_Code, "_131")
  )] <- "PTEN_Kim_131"
  scans$Study_Name[which(
    scans$Study_Name == "PTEN_Kim" & endsWith(scans$Genotype_Code, "_167")
  )] <- "PTEN_Kim_167"
  ...
  scans %<>%
    filter(
      Study_Name %in%
        names(which(
          table(scans$Study_Name, scans$Genotype_Code)[, "WT"] >=
            scanbase_sample_size_threshold
        ))
    )
  ...
  write.csv(
    scans,
    file = glue("{base_directory}/scanbase_scans_autism_filtered_Feb2023.csv")
  )
  ...
  for (i in seq_along(unique_studies)) {
    ...
    if (length(genotypes) >= 1) {
      model_list[[study]] <- list(wildtype = "WT", genotypes = genotypes)
    }
  }

  save(
    list = "model_list",
    file = glue("{base_directory}/model_list_Feb2023.RData")
  )
  return(model_list)
}
...
# Get a mask, to make things faster

MakeEffectSizeMaps <- function(
  jdtype,
  model_list,
  resolution = "200",
  ...
  scans <- read.csv(glue(
    "{base_directory}/scanbase_scans_autism_filtered_Feb2023.csv"
  ))
  ...
  mvol <- mincGetVolume(glue(
    "{dir_determinants}/scanbase_second_level-nlin-3_mask_{resolution}um.mnc"
  ))
  ...
  effect_size_data_matrix <- matrix(
    nrow = length(as.character(unlist(sapply(model_list, "[[", 2)))),
    ncol = nvoxels
  )
  ...
  if (boot == "Y") {
    for (nboot in 1:num_boot) {
      for (model in names(model_list)) {
        ...
        study_scans_wt <- scans %>%
          filter(Study_Name == model, Genotype_Code %in% c("WT"))
        wt_data_matrix <- matrix(nrow = nrow(study_scans_wt), ncol = nvoxels)

        for (i in 1:nrow(study_scans_wt)) {
          filename <- glue(
            "{dir_determinants}/{jddir}/{as.character(study_scans_wt$Mouse_ID)[i]}_{jddir}.mnc"
          )
        ...
        for (genotype in genotypes) {
          ...
          study_scans_mut <- scans %>%
            filter(Study_Name == model, Genotype_Code %in% c(genotype))
          mut_data_matrix <- matrix(
            nrow = nrow(study_scans_mut),
            ncol = nvoxels
          )

          rowboot <- sample(
            nrow(mut_data_matrix),
            size = nrow(mut_data_matrix),
            replace = TRUE
          )
          ...
          for (i in 1:nrow(study_scans_mut)) {
            filename <- glue(
              "{dir_determinants}/{jddir}/{as.character(study_scans_mut$Mouse_ID)[i]}_{jddir}.mnc"
            )
          ...
          filenameES <- glue(
            "{output_phenotype_dir}/{as.character(model)}_{as.character(genotype)}_ES_{jdtype}_{resolution}_{nboot}.mnc"
          )
    ...
    for (model in names(model_list)) {
      ...
      for (genotype in genotypes) {
        print(paste("Working on model:", model, ",", "genotype:", genotype))
        ...
        filenameES <- glue(
          "{output_phenotype_dir}/{as.character(model)}_{as.character(genotype)}_ES_{jdtype}_{resolution}.mnc"
        )
        ...
        effect_size_data_matrix_rownames <- c(
          effect_size_data_matrix_rownames,
          paste(model, genotype, sep = "_")
        )
        k <- k + 1
      }
    }
  ...
  if (boot != "Y") {
    ...
    save(
      file = glue("{output_phenotype_dir}/ES_Matricies_{jdtype}.RData"),
      effect_size_data_matrix
    )
    return(effect_size_data_matrix)
  }
}

#load(file="Data/Outputs/EffectSizeMaps/200/ES_Matricies_Absolute.RData")
#effect_size_data_matrix_abs <- effect_size_data_matrix
#load(file="Data/Outputs/EffectSizeMaps/200/ES_Matricies_Relative.RData")
...
SNFCombineBoot <- function(
  ...
  for (nboot in 1:num_boot) {
    for (i in 1:length(rownames(Data1))) {
      filename_abs <- glue(
        "{boot_effect_dir}/{rownames(Data1)[i]}_ES_Absolute_{resolution}_{nboot}.mnc"
      )
      filename_rel <- glue(
        "{boot_effect_dir}/{rownames(Data2)[i]}_ES_Relative_{resolution}_{nboot}.mnc"
      )
...
CreateClusterAnatomyMaps <- function(
  num_clusters = "10",
  cluster_method = "spectral",
  average_kind = "median",
  ...
  for (j in 2:num_clusters) {
    ...
    for (i in 1:C) {
      ...
      filenameES <- glue(
        "{output_dir}/Group_{i}_Clusternum_{j}_ES_{volume_type}_{resolution}_{average_kind}.mnc"
      )
...
PlotClusterAnatomyMaps <- function(
  num_clusters = "10",
  average_kind = "median",
  volume_type = "absolute",
  resolution = "200",
  ...
  anatVol <- mincGetVolume(glue(
    "{dir_determinants}/scanbase_second_level-nlin-3_{resolution}um.mnc"
  ))
  ...
  for (num_clust in 2:num_clusters) {
    ...
    for (clust in 1:num_clust) {
      ovol <- mincGetVolume(glue(
        "{output_dir}/Group_{clust}_Clusternum_{num_clust}_ES_{volume_type}_{resolution}_{average_kind}.mnc"
      ))
    ...
    cairo_pdf(
      glue(
        "{output_dir}/SliceSeries_{num_clust}_Clusters_{resolution}_{average_kind}_{volume_type}_{slice_dir}.pdf"
      ),
      height = 10,
      width = 5 * as.numeric(num_clusters)
    )
    plt %>% legend("Effect Size") %>% draw()
    dev.off()
  }
}
...
CreateTermsList <- function(
  SFARI_List = "Data/Raw/SFARI-Gene_genes_02-11-2020release_03-02-2020export-1.csv",
...
get_homologs <- function(genes, species, ordered = T) {
  ...
  if (ordered) {
    ...
    for (i in 1:length(genes)) {
      g <- genes[i]
      homologene_ids <- hom$`DB Class Key`[which(
        (hom$Symbol == g) & hom$`Common Organism Name` == species
      )]
      genes_mouse <- c(
        genes_mouse,
        hom$Symbol[which(
          (hom$`DB Class Key` %in% homologene_ids) &
    ...
    homologene_ids <- hom$`DB Class Key`[which(
      (hom$Symbol %in% genes) & hom$`Common Organism Name` == species
    )]
    genes_mouse <- hom$Symbol[which(
      (hom$`DB Class Key` %in% homologene_ids) &
        (hom$`Common Organism Name` == "mouse, laboratory")
    )]
    genes_human <- hom$Symbol[which(
      (hom$`DB Class Key` %in% homologene_ids) &
        (hom$`Common Organism Name` == "human")
    )]
  }
...
GetValidGeneClusterList <- function(
  Cluster_Dir = "Data/Outputs/Clusters",
  boot = "N",
  boot_pick = nboot
) {
  if (boot == "Y") {
  ...
  if (boot == "Y") {
    for (bootpick in 146:nboot) {
      for (num_clusters in 2:total_clusters) {
        all_df <- cl_filt %>%
          dplyr::select(Model, Gene, paste0("group", num_clusters)) %>%
          dplyr::rename(gene = Gene, cluster = paste0("group", num_clusters))

        # Remove duplicates where multiple genes are in the same cluster
        all_df <- all_df[
          !duplicated(
            all_df %>%
              mutate(dup_select = paste(gene, cluster, sep = "-")) %>%
              .$dup_select
          ),
        ]
        ...
        string_ids <- read_tsv(api_call) %>%
          dplyr::select(queryIndex, stringId, preferredName)
        all_df %<>% left_join(string_ids, by = "queryIndex")

        # Remove organism ID
        all_df$fixed_id <- all_df$stringId %>%
          strsplit("\\.") %>%
          map_chr(`[`, 2)
        ...
        write.csv(
          file = glue("{output_dir}/all_gene_data_{bootpick}.csv"),
          x = all_df
        )
        ...
        gx_data %<>%
          left_join(
            all_df %>% dplyr::select(cluster, stringId),
        ...
        write.table(
          base_set,
          file = glue(
            "{output_dir}/base_set_full_neighbourhood_{GeneScore}_{bootpick}.txt"
          ),
          quote = F,
          append = F,
          row.names = F,
        ...
        for (clust in 1:num_clusters) {
          target_set <- gx_data %>%
            filter(cluster_A == clust) %>%
            dplyr::select(preferredName_A, preferredName_B) %>%
            gather %>%
            .$value %>%
            unique
          target_outfile <- glue(
            "{output_dir}/target_set_cluster{num_clusters}_{clust}_{GeneScore}_neighbourhood_{bootpick}.txt"
          )
          write.table(
            target_set,
            file = target_outfile,
          ...
          target_set_only_models <- all_df %>%
            filter(cluster == clust) %>%
            .$gene
          target_outfile_only_models <- glue(
            "{output_dir}/target_set_cluster{num_clusters}_{clust}_{GeneScore}_only_models_{bootpick}.txt"
          )
          write.table(
            target_set_only_models,
            file = target_outfile_only_models,
            quote = F,
            append = F,
            row.names = F,
            col.names = F
        ...
        for (clust in 1:num_clusters) {
          target_set <- gx_data %>%
            ...
            unique
          target_outfile <- glue(
            "{output_dir}/target_set_cluster{num_clusters}_Not_{clust}_{GeneScore}_neighbourhood_{bootpick}.txt"
          )
          write.table(
            target_set,
            file = target_outfile,
          ...
          target_set_only_models <- all_df %>%
            ...
            .$gene
          target_outfile_only_models <- glue(
            "{output_dir}/target_set_cluster{num_clusters}_Not_{clust}_{GeneScore}_only_models_{bootpick}.txt"
          )
          write.table(
            target_set_only_models,
            file = target_outfile_only_models,
      ...
      listfname <- glue(
        "{output_dir}/gene_interaction_tables_for_all_clusters_{GeneScore}_{bootpick}.RData"
      )
    ...
    for (num_clusters in 2:total_clusters) {
      ...
      write.table(
        base_set,
        file = glue("{output_dir}/base_set_full_neighbourhood_{GeneScore}.txt"),
        quote = F,
        append = F,
        row.names = F,
        col.names = F
      ...
      for (clust in 1:num_clusters) {
        target_set <- gx_data %>%
          ...
          unique
        target_outfile <- glue(
          "{output_dir}/target_set_cluster{num_clusters}_{clust}_{GeneScore}_neighbourhood.txt"
        )
        write.table(
          target_set,
          file = target_outfile,
        ...
        target_outfile_only_models <- glue(
          "{output_dir}/target_set_cluster{num_clusters}_{clust}_{GeneScore}_only_models.txt"
        )
        write.table(
          target_set_only_models,
          file = target_outfile_only_models,
      ...
      for (clust in 1:num_clusters) {
        target_set <- gx_data %>%
          ...
          unique
        target_outfile <- glue(
          "{output_dir}/target_set_cluster{num_clusters}_Not_{clust}_{GeneScore}_neighbourhood.txt"
        )
        write.table(
          target_set,
          file = target_outfile,
        ...
        target_set_only_models <- all_df %>% filter(cluster != clust) %>% .$gene
        target_outfile_only_models <- glue(
          "{output_dir}/target_set_cluster{num_clusters}_Not_{clust}_{GeneScore}_only_models.txt"
        )
        write.table(
          target_set_only_models,
          file = target_outfile_only_models,
    ...
    listfname <- glue(
      "{output_dir}/gene_interaction_tables_for_all_clusters_{GeneScore}.RData"
    )
    save(list = c("all_df_list", "gx_data_list"), file = listfname)
  }
}
...
GetNeighbourhoodEnrichment <- function(
  GeneScore = "950",
  total_clusters = "10",
  ...
  if (boot == "Y") {
    ...
    all_df <- cl_filt %>%
      dplyr::select(Model, Gene, paste0("group", total_clusters)) %>%
      dplyr::rename(gene = Gene, cluster = paste0("group", total_clusters))
    all_df <- all_df[
      !duplicated(
        all_df %>%
          mutate(dup_select = paste(gene, cluster, sep = "-")) %>%
    ...
    for (bootpick in 1:nboot) {
      ...
      base_brain <- read.csv(
        "Data/Raw/sagittal_gene_table_normalized_filtered.csv",
        header = T
      )$msg.genes.acronym %>%
        as.character
      base_models_neighbourhood <- read.csv(
        glue(
          "{Neighbour_dir}/{GeneScore}/Boot/base_set_full_neighbourhood_{GeneScore}_{bootpick}.txt"
        ),
        header = F
      )$V1 %>%
        as.character
      ...
      base <- list(
        brain = base_brain,
        models_neighbourhood = base_models_neighbourhood,
        models_only = base_models_only
      )
      gmt_file <- Bader_List
      MappingsGMT <- tmodImportMSigDB(gmt_file, format = "gmt")
      ...
      for (num_clusters in 2:total_clusters) {
        ...
        for (clust in 1:num_clusters) {
          if (
            file.size(glue(
              "{Neighbour_dir}/{GeneScore}/Boot/target_set_cluster{num_clusters}_{clust}_{GeneScore}_neighbourhood_{bootpick}.txt"
            )) ==
              0
          ) {
            next
          } else {
            target_set_cl <- read.csv(
              glue(
                "{Neighbour_dir}/{GeneScore}/Boot/target_set_cluster{num_clusters}_{clust}_{GeneScore}_neighbourhood_{bootpick}.txt"
              ),
              header = F
            ...
            target_set_cl <- read.csv(
              glue(
                "{Neighbour_dir}/{GeneScore}/Boot/target_set_cluster{num_clusters}_{clust}_{GeneScore}_only_models_{bootpick}.txt"
              ),
              header = F
            )$V1 %>%
              as.character
        ...
        for (clust in 1:num_clusters) {
          ...
          if (length(target_set) == 0) {
            ...
            fg <- get_homologs(
              as.character(target_set),
              "mouse",
              ordered = F
            )$human_genes
            bg <- get_homologs(
              as.character(base_set),
              "mouse",
              ordered = F
            )$human_genes
            ...
            write.csv(
              tmodObj,
              file = glue(
                "{output_dir}/NewBader_enrichment_clusterneighbourhood_vs_brain_all_{num_clusters}_{clust}_{GeneScore}_{bootpick}.csv"
              ),
              row.names = F
            )
          }
    ...
    base_models_neighbourhood <- read.csv(
      glue(
        "{Neighbour_dir}/{GeneScore}/base_set_full_neighbourhood_{GeneScore}.txt"
      ),
      header = F
    )$V1 %>%
      as.character
    ...
    for (num_clusters in 2:total_clusters) {
      ...
      for (clust in 1:num_clusters) {
        if (
          file.size(glue(
            "{Neighbour_dir}/{GeneScore}/target_set_cluster{num_clusters}_{clust}_{GeneScore}_neighbourhood.txt"
          )) ==
            0
        ) {
          next
          ...
          target_set_cl <- read.csv(
            glue(
              "{Neighbour_dir}/{GeneScore}/target_set_cluster{num_clusters}_{clust}_{GeneScore}_only_models.txt"
            ),
            header = F
          )$V1 %>%
            as.character
      ...
      for (clust in 1:num_clusters) {
        ...
        if (length(target_set) == 0) {
          ...
          write.csv(
            tmodObj,
            file = glue(
              "{output_dir}/NewBader_enrichment_clusterneighbourhood_vs_brain_all_{num_clusters}_{clust}_{GeneScore}.csv"
            ),
            row.names = F
          )
        }
...
# Enrichment tests
# First get different base sets
GetSingleGeneNeighbourhood <- function(
  GeneScore = "400",
  total_clusters = "10",
  ...
  all_df <- cl_filt %>%
    dplyr::select(Model, Gene) %>%
    dplyr::rename(gene = Gene)

  # Remove duplicates where multiple genes are in the same cluster
  all_df <- all_df[
    !duplicated(
      all_df %>% mutate(dup_select = paste(gene, sep = "-")) %>% .$dup_select
    ),
  ]
  ...
  gx_data %<>%
    left_join(
      all_df %>% dplyr::select(gene, stringId),
      by = c("stringId_A" = "stringId")
    )
  ...
  for (sgene in 1:nrow(all_df)) {
    target_set <- gx_data %>%
      filter(gene == all_df$gene[sgene]) %>%
      dplyr::select(preferredName_A, preferredName_B) %>%
      gather %>%
      .$value %>%
      unique
    target_outfile <- glue(
      "{output_dir}/target_set_cluster_{all_df$gene[sgene]}_{GeneScore}_neighbourhood.txt"
    )
    write.table(
      target_set,
      file = target_outfile,
...
GetSingleGeneNeighbourhoodEnrichment <- function(
  GeneScore = "400",
  total_clusters = "10",
  Bader_List = "Data/Raw/Human_Reactome_March_01_2020_symbol.gmt",
  Neighbour_dir = "Data/Outputs/NeighbourhoodInfo_Paper/",
  ...
  for (SingleGene in 1:nrow(all_df)) {
    ...
    if (
      file.size(
        file = glue(
          "{Neighbour_dir}/{GeneScore}/target_set_cluster_{sgene}_{GeneScore}_neighbourhood.txt"
        )
      ) ==
        0
    ) {
    ...
    write.csv(
      tmodObj,
      file = glue(
        "{output_dir}/NewBader_enrichment_singlegeneneighbourhood_vs_brain_all_{sgene}_{GeneScore}.csv"
      ),
      row.names = F
    )
  }
...
GetNeighbourhoodEnrichment_NotCluster <- function(
  GeneScore = "900",
  total_clusters = "10",
  Bader_List = "Data/Raw/Human_Reactome_March_01_2020_symbol.gmt",
    ...
    for (clust in 1:num_clusters) {
      ...
      if (length(target_set) == 0) {
        ...
        write.csv(
          tmodObj,
          file = glue(
            "{output_dir}/NewBader_enrichment_clusterneighbourhood_vs_brain_all_{num_clusters}_Not_{clust}_{GeneScore}.csv"
          ),
          row.names = F
        )
      }
...
ClusterPairwiseComp <- function(
  ...
  base_brain <- read.csv(
    ...
    as.character
  base_models_neighbourhood <- read.csv(
    glue("{TargetSet_Dir}/base_set_full_neighbourhood_{GeneScore}.txt"),
    header = F
  )$V1 %>%
    as.character
  base_models_only <- all_df$gene
  ...
  SFARI <- read.csv(
    "Data/Raw/SFARI-Gene_genes_02-11-2020release_03-02-2020export-1.csv"
  )
  gmt_file <- Bader_List
  MappingsGMT <- tmodImportMSigDB(gmt_file, format = "gmt")
  ...
  for (num_clusters in 2:total_clusters) {
    ...
    for (i in 1:num_clusters) {
      ...
      if (
        file.size(glue(
          "{TargetSet_Dir}/target_set_cluster{num_clusters}_{clust}_{GeneScore}_neighbourhood.txt"
        )) ==
          0
      ) {
        next
      } else {
        result_all_clusters[[i]] <- read.csv(
          file = glue(
            "{Enrichment_Dir}/NewBader_enrichment_clusterneighbourhood_vs_brain_all_{num_clusters}_{clust}_{GeneScore}.csv"
          )
        )
        target_set[[clust]] <- read.csv(
          file = glue(
            "{TargetSet_Dir}/target_set_cluster{num_clusters}_{clust}_{GeneScore}_neighbourhood.txt"
          ),
          header = F
      ...
      for (ip in 2:length(ok_clust)) {
        for (jp in 1:(i - 1)) {
          ...
          ctable <- matrix(
            c(bi, bj, Bi - bi, Bj - bj),
            nrow = 2,
            dimnames = list(paste("Cluster", c(i, j)), c("b", "B-b"))
          )
          ...
          if (any(c(any(rowSums(ctable) == 0), any(colSums(ctable) == 0)))) {
            out_term_df_ij <- data.frame(
              term = terms[k],
              Title = unique(out_term$Title),
              cluster_i = i,
              cluster_j = j,
              test = paste0(
                "Prob[proportion(Cluster ",
                i,
                ") > proportion(Cluster ",
                j,
                ")]"
            ...
            ctest <- exact.test(
              ...
              to.plot = F,
              ref.pvalue = TRUE
            ) # Use two-sided tests so we can compare i>j and j>i
            ctest$statistic
            ctest$estimate
            ctest$p.value
            ctest$parameter
    ...
    results_pairwise_tests_differences <- results_pairwise_tests %>%
      mutate(adj.P.Val = p.adjust(P.Value, method = "fdr"))
    results_pairwise_tests_differences <- results_pairwise_tests_differences %>%
    ...
    table(c(
      results_pairwise_tests_differences$cluster_i,
      results_pairwise_tests_differences$cluster_j
    ))

    # Venn
    results_pairwise_tests_clustered <- results_pairwise_tests %>%
      filter(adj.P.Val <= 0.05)
    results_pairwise_tests_clustered %<>%
      mutate(
        A = ifelse(estimate > 0, cluster_i, cluster_j),
        B = ifelse(estimate < 0, cluster_i, cluster_j),
        estimate = abs(estimate)
      ) %>%
      dplyr::select(term, Title, A, B, estimate, P.Value, adj.P.Val)
    write.csv(
      x = results_pairwise_tests_clustered,
      file = glue(
        "{output_dir}/NewBader_pairwise_tests_significant_ordered_{num_clusters}.csv"
      )
    )
    ...
    A_count_table <- table(
      results_pairwise_tests_clustered_counting$Title,
      results_pairwise_tests_clustered_counting$A
    )
    ok_title_A <- which(A_count_table == (length(ok_clust) - 1), arr.ind = T)
    A_counts <- paste(
      rownames(A_count_table)[ok_title_A[, "row"]],
      colnames(A_count_table)[ok_title_A[, "col"]],
      sep = "_"
    )
    results_pairwise_tests_clustered_majordiff <- results_pairwise_tests_clustered %>%
      mutate(A_count = paste(Title, A, sep = "_")) %>%
      filter(A_count %in% A_counts) %>%
      dplyr::select(-A_count)
    write.csv(
      x = results_pairwise_tests_clustered_majordiff,
      file = glue(
        "{output_dir}/NewBader_pairwise_tests_significant_ordered_{num_clusters}_majordiff.csv"
      )
    )
  }
}
...
ClusterPairwiseCompvsALL <- function(
  GeneScore = gscore,
  total_clusters = "10",
  Bader_List = "Data/Raw/Human_Reactome_March_01_2020_symbol.gmt",
  TargetSet_Dir = "Data/Outputs/NeighbourhoodInfo",
  ...
  for (num_clusters in 2:total_clusters) {
    ...
    for (i in 1:num_clusters) {
      ...
      if (
        ...
        result_not_clusters[[i]] <- read.csv(
          file = glue(
            "{Enrichment_Dir}/NewBader_enrichment_clusterneighbourhood_vs_brain_all_{num_clusters}_Not_{clust}_{GeneScore}.csv"
          )
        )
        target_set[[clust]] <- read.csv(
          file = glue(
        ...
        target_set_not[[clust]] <- read.csv(
          file = glue(
            "{TargetSet_Dir}/target_set_cluster{num_clusters}_Not_{clust}_{GeneScore}_neighbourhood.txt"
          ),
          header = F
        )$V1
      }
    ...
    for (clust in ok_clust) {
      ...
      enrichment_object <- MappingsGMT
      fg <- get_homologs(
        as.character(target_set_not_cl),
        "mouse",
        ordered = F
      )$human_genes
      bg <- get_homologs(
      ...
      if (exists("result_individual_not_clusters")) {
        result_individual_not_clusters <- rbind(
          result_individual_not_clusters,
          tmodObj
        )
      } else {
    ...
    for (k in 1:length(terms)) {
      ...
      for (ip in 1:length(ok_clust)) {
        ...
        ctable <- matrix(
          ...
          nrow = 2,
          dimnames = list(
            c(paste("Cluster", i), paste("Not Cluster", i)),
            c("b", "B-b")
          )
        )
        #ctable <- matrix(c(bi/ni, bj/nj, (Bi/Ni)-(bi/ni), (Bj/Nj)-(bj/nj)), nrow=2, dimnames = list(paste("Cluster", c(i,j)), c("Proportion of input genes in module", "Proportion of GO genes in base")))
    ...
    write.csv(
      x = results_pairwise_tests_differences,
      file = glue(
        "{output_dir}/NewBader_pairwise_tests_significant_ordered_{num_clusters}.csv"
      )
    )
...
SummarizeAcrossScores <- function(
  Pairwise_Dir = "Data/Outputs/ClusterPairwiseComp",
  total_clusters = "10"
) {
  score_list <- list()
  for (gscore in c(400, 700, 900)) {
    for (num_clusters in 2:total_clusters) {
      dat <- read.csv(glue(
        "{Pairwise_Dir}/{gscore}/NewBader_pairwise_tests_significant_ordered_{num_clusters}_majordiff.csv"
      ))
