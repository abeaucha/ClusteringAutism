...
df_neighbourhood
plt_neighbourhood <- ggplot(df_neighbourhood, aes(x = score, y = size, group = gene)) + 
  geom_line(alpha = 0.5) + 
  geom_point(col = "grey30") +
  labs(x = "StringDB score",
...
df_size_props <- expand_grid(score = gene_scores,
                             size = size_seq,
                             prop = 0)

for (i in 1:nrow(df_size_props)) {
  df_size_props[[i, "prop"]] <- df_neighbourhood %>% 
    filter(score == df_size_props[[i, "score"]], 
           size <= df_size_props[[i, "size"]]) %>% 
    nrow()/length(genes_unique)
}

plt_neighbourhood_prop <- ggplot(df_size_props, aes(x = size, y = prop, group = factor(score), col = factor(score))) + 
  geom_line() + 
  geom_point() + 
  scale_x_continuous(breaks = size_seq) + 
  scale_y_continuous(breaks = seq(0, 1, by = 0.1)) + 
  labs(x = "Neighbourhood size",
