...
sickkids <- read_csv(sickkids, show_col_types = FALSE) %>%
  ...
  mutate(Dataset = "SickKids", file = str_c(Subject_ID, file_suffix))
...
pond_dbm <- read_csv(pond_dbm, show_col_types = FALSE) %>%
  select(Subject_ID, Scan_ID, Rating_Motion, Rating_IN3, QC_Status) %>%
  mutate(Dataset = "POND")
...
pond_metadata <- read_excel(pond_metadata) %>%
  ...
  select(Subject_ID, Sex = NSI_SEX, DX = PRIMARY_DIAGNOSIS) %>%
  mutate(
...
pond_neuro <- read_csv(pond_neuro, show_col_types = FALSE) %>%
  ...
  mutate(
    Subject_ID = str_replace(Subject_ID, "sub-", "POND_"),
