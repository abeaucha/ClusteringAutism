...
hbn_basic <- hbn_basic_files %>%
  ...
  map(function(x) {
    x %>%
      select(Subject_ID = participant_id, Sex, Age)
  }) %>%
  bind_rows(.id = "Site") %>%
...
if (dx_group == "NDD") {
  ...
  hbn_dx_include <- c("Anxiety Disorders", "No Diagnosis Given")
  hbn_dx_exclude <- c("")
  ...
  hbn_dx_labels <- hbn_dx %>%
    ...
    select(DX_01) %>%
    distinct() %>%
    mutate(DX = ifelse(DX_01 == "No Diagnosis Given", "Control", DX_01))
  ...
  hbn_dx_include <- c("Depressive Disorders", "No Diagnosis Given")
  hbn_dx_exclude <- c("")
...
hbn_dx <- hbn_dx %>%
  select(Subject_ID = EID, DX_01_Cat, DX_01) %>%
  filter(
    DX_01 %in% hbn_dx_include,
    !(DX_01 %in% hbn_dx_exclude),
    !is.na(DX_01)
  ) %>%
  arrange(DX_01_Cat, DX_01)
...
hbn <- left_join(hbn_imgs, hbn_dx, by = "Subject_ID") %>%
  mutate(Dataset = "HBN", Scanner = "")
...
hbn <- hbn %>%
  filter(!is.na(DX), !is.na(Sex), !is.na(Age), !is.na(Site), !is.na(Scanner))

ids_to_remove <- c(
  "sub-NDAREK255DEE_acq-HCP_T1w",
  "d8_0033_01",
  "d8_0034_02",
  "d8_0035_01",
  "d8_0036_01",
...
hbn <- hbn %>%
  select(Subject_ID, Scan_ID, Dataset, Age, Sex, DX, Site, Scanner, file) %>%
