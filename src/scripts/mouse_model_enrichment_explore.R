...
enrichment_E <- enrichment_NLQ <- enrichment_binary <- matrix(
  data = 0,
  nrow = n_models,
  ncol = n_modules
)
...
for (l in 1:length(list_enrichment)) {
  ...
  for (gene in genes_unique) {
    ...
    neighbourhood <- get_gene_neighbourhood(
      genes = gene,
      score = gene_scores[l],
    ...
    if (nrow(neighbourhood) == 0) {
      ...
      target_set <- unique(c(
        neighbourhood[["gene_A"]],
        neighbourhood[["gene_B"]]
      ))
    }

    enrichment_gene <- get_neighbourhood_enrichment(
      target = target_set,
...
similarity_network_new <- function(
  x,
  metric = "correlation",
  K = 10,
  sigma = 0.5,
...
idx_single_genes <- rownames(effect_size_data_matrix_abs) %in%
  str_remove(models[["file"]], ".mnc")
...
for (l in 1:length(list_clusters)) {
  affinity <- similarity_network_new(
    x = list(es_matrix_abs, es_matrix_rel, list_enrichment[[l]][["E"]]),
    metric = "euclidean"
  )
  # affinity <- similarity_network_new(x = list(es_matrix_rel, list_enrichment[[l]][["E"]]),
  #                                    metric = "euclidean")
  list_metrics[[l]][["E"]] <- estimate_cluster_metrics(
    W = affinity,
    NUMC = 2:10
  )
  ...
  affinity <- similarity_network_new(
    x = list(es_matrix_abs, es_matrix_rel, list_enrichment[[l]][["NLQ"]]),
    metric = "euclidean"
  )
  # affinity <- similarity_network_new(x = list(es_matrix_rel, list_enrichment[[l]][["NLQ"]]),
  #                                    metric = "euclidean")
  list_metrics[[l]][["NLQ"]] <- estimate_cluster_metrics(
    W = affinity,
    NUMC = 2:10
  )
  ...
  affinity <- similarity_network_new(
    x = list(es_matrix_abs, es_matrix_rel, list_enrichment[[l]][["binary"]]),
    metric = "euclidean"
  )
  # affinity <- similarity_network_new(x = list(es_matrix_rel, list_enrichment[[l]][["binary"]]),
  #                                    metric = "euclidean")
  list_metrics[[l]][["binary"]] <- estimate_cluster_metrics(
    W = affinity,
    NUMC = 2:10
  )
...
plt_metrics <- ggplot(
  df_metrics,
  aes(x = nk, y = eigengap, group = measure, col = measure)
) +
  geom_line() +
  geom_point() +
...
df_metrics %>%
  ...
  geom_point() +
  scale_x_continuous(breaks = seq(2, 10, by = 1)) +
  labs(y = "Eigengap", col = "StringDB score") +
  theme_bw() +
  theme(panel.grid.minor.x = element_blank())
dev.off()
...
for (score in gene_scores) {
  for (measure in measures) {
    ...
    sankey <- ggplot(
      clusters_long,
      aes(x = nk, stratum = k, alluvium = ID, fill = k, label = k)
    ) +
...
affinity_euclidean <- similarity_network_new(
  x = list(es_matrix_abs, es_matrix_rel),
  metric = "euclidean"
)

affinity_cor <- similarity_network_new(
  x = list(es_matrix_abs, es_matrix_rel),
  metric = "correlation"
)
df_metrics <- bind_rows(
  estimate_cluster_metrics(W = affinity_euclidean, NUMC = 2:10) %>%
...
plt_metrics <- ggplot(
  df_metrics,
  aes(x = nk, y = eigengap, group = metric, col = metric)
) +
  geom_line() +
  geom_point() +
  scale_x_continuous(breaks = seq(2, 10, by = 1)) +
...
affinity_euclidean <- similarity_network_new(
  x = list(effect_size_data_matrix_abs, effect_size_data_matrix_rel),
  metric = "euclidean"
)
