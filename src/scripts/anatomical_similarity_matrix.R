...
option_list <- list(
  make_option('--mouse-dir',
              type = 'character',
              help = paste("Path to direcotry")),
  make_option("--mouse-labels",
              type = "character",
              help = paste("")),
  make_option("--mouse-mask",
              type = "character",
              help = paste("")),
  make_option("--mouse-tree",
              type = "character",
              help = paste("")),
  make_option('--human-dir',
              type = 'character',
              help = paste("Path to direcotry")),
  make_option("--human-tree",
              type = "character",
              help = paste("")),
  make_option("--human-metadata",
              type = "character",
              help = paste("")),
  make_option("--tree-labels",
              type = "character",
              help = paste("")),
  make_option('--metric',
              type = 'character',
...
path_func_tree <- file.path(working_dir,
                            script_dir,
                            "functions",
                            "tree_tools.R")
source(path_func)
...
compute_anatomical_similarity <- function(latent_space, mouse_files, human_files, mouse_tree, mouse_labels, mouse_mask, human_tree, human_samples, metric, save_intermediate = FALSE, outfile = "similarity.csv") {
  
  mouse_file <- str_subset(mouse_files, str_c("mousetransform_", latent_space, ".csv"))
  human_file <- str_subset(human_files, str_c("humantransform_", latent_space, ".csv"))
  ...
  if (save_intermediate) {
    data.table::fwrite(x = as_tibble(similarity, rownames = "Human"),
                       file = outfile)
  }
...
if (inparallel) {
  ...
  similarity <- foreach(i = 1:length(latent_space_ids),
                        .packages = c("tidyverse", "RMINC", "data.tree"),
                        .options.snow = opts) %dopar% {
                          compute_anatomical_similarity(latent_space = latent_space_ids[i],
                                                        ...
                                                        outfile = str_replace(outfile, ".csv", str_c("_", latent_space_ids[i], ".csv")))
                        }
  close(pb)
  stopCluster(cl)
} else {
  similarity <- foreach(i = 1:length(latent_space_ids),
                        .packages = c("tidyverse", "RMINC", "data.tree")) %do% {
                          progress(n = i)
                          compute_anatomical_similarity(latent_space = latent_space_ids[i],
                                                        mouse_files = mouse_files,
                                                        human_files = human_files,
...
similarity_array <- array(unlist(similarity),
                          dim = c(nrow(similarity[[1]]),
                                  ncol(similarity[[1]]),
                                  length(similarity)))
...
data.table::fwrite(x = as_tibble(similarity_avg, rownames = "Human"),
                   file = outfile)