...
pruneAtNode <- function(node) {
  if (is.null(node[["cutPoints"]])) {
    stop(paste(
      "Node",
      node[["name"]],
      "has no field 'cutPoints'. Cut points can be attached using the function attachCutPoints()."
    ))
  }
...
pruneAnatTree <- function(anatTree, nodes, method = "AtNode") {
  ...
  if (method == "AtNode") {
    ...
    stop(paste(
      "Argument `method` must be one of 'AtNode' or 'BelowNode'. Got",
      method
    ))
  }
...
prune_tree <- function(
  ...
  if (verbose) {
    msg <- paste("Tree pruned", method, "nodes:", paste(nodes, collapse = ", "))
    message(msg)
  }
