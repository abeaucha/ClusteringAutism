...
estimate_cluster_metrics <- function(W, NUMC = 2:5) {
  ...
  if (length(NUMC) <= 0) {
    warning(paste(
      "Invalid NUMC provided, must be an integer vector",
      "with atleast one other number than 1.",
      "Using default NUMC=c(2,3,4,5)",
      sep = ""
    ))
    NUMC <- 2:5
  }
  ...
  for (c_index in 1:length(NUMC)) {
    ...
    temp1 <- EigenVectors[
      do.call(
        order,
        lapply(1:ncol(EigenVectors), function(i) EigenVectors[, i])
      ),
    ]
...
compare_centroid_anatomy <- function(
  centroid_dirs,
  species,
  nk,
  k,
  ...
  for (i in 1:2) {
    ...
    for (s in names(freq_signed)) {
      freq_signed[[s]] <- compute_cluster_fractions(
        cluster_dir = centroid_dirs[[i]],
        nk = nk[[i]],
        k = k[[i]],
        labels = atlas[["labels"]],
        defs = atlas[["defs"]],
  ...
  freq <- intersect_neuro_homologues(
    x = freq,
    species = species,
    homologues = nodes
  )
...
compute_cluster_fractions <- function(
  cluster_dir,
  nk,
  k,
  labels,
  ...
  threshold_symmetric = NULL,
  threshold_comparison = NULL
) {
  #Import cluster map
  cluster_map <- import_cluster_map(
    imgdir = cluster_dir,
    mask = mask,
    nk = nk,
    k = k,
  ...
  cluster_fractions <- tabulate_labels_in_img(
    img = cluster_map,
    mask = mask,
    labels = labels,
    defs = defs,
...
compute_enrichment_HGtest <- function(
  mouse_enrichment,
  human_enrichment,
  alpha_mouse,
  alpha_human = NULL
  ...
  df_hgtest <- expand_grid(
    alpha_mouse = alpha_mouse,
    alpha_human = alpha_human
  ) %>%
    mutate(b = 0, B = 0, n = 0, N = N, E = 0, pval = 0)
  for (i in 1:nrow(df_hgtest)) {
    mouse_pass <- mouse_enrichment[["adj.P.Val"]] <
      df_hgtest[[i, "alpha_mouse"]]
    human_pass <- human_enrichment[["adj.P.Val"]] <
      df_hgtest[[i, "alpha_human"]]
    B <- sum(mouse_pass)
    n <- sum(human_pass)
...
compute_enrichment_PR <- function(
  mouse_enrichment,
  human_enrichment,
  alpha = 0.05
) {
  ...
  for (i in 1:length(alpha)) {
    ...
    if (sum(response) > 0) {
      ...
      out[[i]] <- PRROC::pr.curve(
        scores.class0 = predictor[response],
...
compute_enrichment_ROC <- function(
  mouse_enrichment,
  human_enrichment,
  alpha = 0.05
) {
...
simulate_enrichment_null <- function(
  enrichment,
  n_true,
  method = "ROC",
  n_sim = 500
) {
  if (!(method %in% c("ROC", "PR", "HG"))) {
    stop()
  }
  ...
  for (i in 1:length(n_true)) {
    ...
    for (s in 1:n_sim) {
      ...
      if (method == "ROC") {
        out_i[[s]] <- compute_enrichment_ROC(
          mouse_enrichment = target,
        ...
        out_i[[s]] <- compute_enrichment_PR(
          ...
          map_dfr(
            .f = function(x) {
              tibble(AUC = x[["auc.integral"]], Prop = x[["true.proportion"]])
            },
            .id = "alpha"
          ) %>%
          mutate(n_enriched = n_true[i]) %>%
        ...
        out_i[[s]] <- compute_enrichment_HGtest(
          mouse_enrichment = target,
          human_enrichment = enrichment,
          alpha_mouse = 0.05,
...
compute_similarity_significance <- function(
  similarity,
  permutations,
  off_diag = 1
) {
    ...
    for (nk_2 in (nk_1 - off_diag):(nk_1 + off_diag)) {
      if ((nk_2 > 1) & (nk_2 <= nk_max_2)) {
        sim_nk <- similarity %>%
          ...
          filter(img1_nk == nk_1, img2_nk == nk_2) %>%
          mutate(pval = 0)
...
export_pdf <- function(x, file, width, height, units = "in") {
  ...
  tryCatch(
    {
      pdf(file = file, width = unit(width, "in"), height = unit(height, "in"))
      grid.draw(x)
      dev.off()
    },
    error = function(c) {
...
import_enrichment_human <- function(
  params_id,
  ...
  bader_version = 2025,
  file_prefix = "cluster_pathway_enrichment"
) {
...
import_enrichment_mouse <- function(
  params_id,
  pipeline_dir = "data/human/derivatives/v3/",
  nk,
  k,
...
import_similarity <- function(
  ...
  similarity <- read_csv(input_file, show_col_types = FALSE) %>%
    ...
    unite(col = "img2_cluster_id", img2_nk, img2_k, sep = "-", remove = FALSE)
...
import_similarity_permutations <- function(
  param_id,
  pipeline_dir,
  combine_jacobians = TRUE
) {
...
prepare_radar_chart <- function(
  ...
  for (s in species) {
    ...
    fractions_positive <- compute_cluster_fractions(
      cluster_dir = cluster_dirs[[s]],
      nk = nk[[s]],
      k = k[[s]],
      labels = atlas[["labels"]],
      defs = atlas[["defs"]],
    ...
    fractions_negative <- compute_cluster_fractions(
      cluster_dir = cluster_dirs[[s]],
      nk = nk[[s]],
      k = k[[s]],
      labels = atlas[["labels"]],
    ...
    fractions_positive <- select(
      fractions_positive,
      name,
      positive = f_per_label
    )
    fractions_negative <- select(
      fractions_negative,
      name,
      negative = f_per_label
    )

    fractions[[s]] <- inner_join(
      fractions_positive,
      fractions_negative,
      by = "name"
    ) %>%
      mutate(negative = -1 * negative, species = s)
  }

  #Intersect regions with neuroanatomical homologues
  fractions <- intersect_neuro_homologues(x = fractions, homologues = spokes)
...
reduce_human_labels <- function(tree, labels, defs) {
  defs_reduced <- reduce_human_defs(tree = tree, defs = defs, simplify = FALSE)

  ind_match <- match(x = labels, table = defs_reduced[["sample_label"]])
...
reduce_human_atlas <- function(tree, labels, defs, nodes, remove = TRUE) {
  ...
  #Reduce atlas labels
  labels_reduced <- reduce_human_labels(
    tree = tree_reduced,
    labels = labels,
    defs = defs
  )
  ...
  defs_reduced <- reduce_human_defs(
    tree = tree_reduced,
    defs = defs,
    simplify = TRUE
  )

  return(list(labels = labels_reduced, defs = defs_reduced))
}
...
#' the image.
tabulate_labels_in_img <- function(
  img,
  labels,
  defs,
  mask = NULL,
  ...
  if (length(labels_in_img) != 0) {
    out <- table(labels_in_img) %>%
      as_tibble() %>%
      rename(label = labels_in_img, n_per_label = n) %>%
      mutate(label = as.integer(label))
  } else {
    out <- tibble(label = defs[["label"]], n_per_label = 0)
  }
  ...
  out <- out %>%
    ...
    mutate(
      n_per_label = ifelse(is.na(n_per_label), 0, n_per_label),
