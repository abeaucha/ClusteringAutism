...
fig2_ss_widths <- c(rep(fig2_ss_width_1, nk_human), fig2_padding_width, rep(fig2_ss_width_1, nk_mouse))
fig2_ss_heights <- fig2_ss_height

fig2_ss_widths_in <- unit(fig2_ss_widths, "inch")
fig2_ss_heights_in <- unit(fig2_ss_heights, "inch")

fig2_ss_grobs_list <- c(fig2_ss_human_grobs,
                        list(rectGrob(gp = gpar(fill = "black"))),
                        fig2_ss_mouse_grobs)
...
```{r}
fig2_ss_human_grobs <- sliceSeries(nrow = ss_nslices, ncol = 1, slices = human_slices) %>%
  anatomy(human_anat_vol_cropped, low = human_anat_low, high = human_anat_high) %>% 
  overlay(list_human_centroids[["relative"]][[1]],
          low = overlay_low, high = overlay_high,
          symmetric = TRUE) %>% 
  sliceSeries(nrow = ss_nslices, ncol = 1, slices = human_slices) %>%
  anatomy() %>% 
...
grid.newpage()
grid.draw(fig2_ss_human_grobs)
```


```{r}
fig2_ss_mouse_grobs <- sliceSeries(nrow = ss_nslices, ncol = 1, slices = mouse_slices) %>% 
  anatomy(mouse_anat_vol, low = mouse_anat_low, high = mouse_anat_high) %>% 
  overlay(list_mouse_centroids[["relative"]][[1]], 
          low = overlay_low, high = overlay_high, 
          symmetric = TRUE) %>% 
  sliceSeries(nrow = ss_nslices, ncol = 1, slices = mouse_slices) %>% 
  anatomy() %>% 
  ...
  sliceSeries(nrow = ss_nslices, ncol = 1, slices = mouse_slices) %>% 
  anatomy() %>% 
  overlay(list_mouse_centroids[["relative"]][[4]], 
          low = overlay_low, high = overlay_high, 
          symmetric = TRUE) %>% 
  grobify()

grid.newpage()
grid.draw(fig2_ss_mouse_grobs)
```
...
fig2_ss_mouse_width <- fig2_ss_width_nopadding*nk_mouse/(nk_mouse + nk_human)

fig2_ss_human_width <- fig2_ss_width_nopadding*nk_human/(nk_mouse + nk_human)

fig2_ss_height <- fig2_ss_width_nopadding*ss_nslices/(nk_mouse + nk_human)

fig2_ss_widths <- c(fig2_ss_human_width, 0.2, fig2_ss_mouse_width)
fig2_ss_heights <- c(fig2_ss_height)

fig2_ss_widths_in <- unit(fig2_ss_widths, "inch")
fig2_ss_heights_in <- unit(fig2_ss_heights, "inch")

fig2_ss_grob <- arrangeGrob(fig2_ss_human_grobs,
                            rectGrob(),
                            fig2_ss_mouse_grobs,
