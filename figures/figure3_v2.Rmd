...
fig3_heatmap_padding_height_1_pt <- 21
fig3_heatmap_padding_height_2_pt <- 19
# fig3_heatmap_padding_height_3_pt <- 19
...
fig3_bottom_ss_padding_pt <- 2

# Padding between panels
fig3_bottom_panel_padding_pt <- 4

# Total padding width 
fig3_ss_padding_width_pt <- fig3_bottom_panel_padding_pt*(nss-1) + fig3_bottom_ss_padding_pt*((nslc_tot - 1) - (nss-1))
...
for (i in 1:length(list_mouse_slices)) {
  ...
  for (j in 1:nslc) {
    ...
    fig3_bottom_ss_heights <- c(fig3_human_ss_height_pt, fig3_bottom_ss_padding_pt, fig3_mouse_ss_height_pt)
    ...
    ss_border <- rectGrob(gp = gpar(fill = NA, col = neuro_palette_nk[j], lwd = 1.75))
    
    outfile <- paste(output_plot_prefix, "bottom_ss", names(list_mouse_slices[[i]])[j], sep = "_")
    outfile <- paste0(outfile, ".pdf")
    outfile <- file.path(output_dir, outfile)
    export_pdf(x = ss,
               file = outfile,
               width = sum(fig3_bottom_ss_widths),
               height = sum(fig3_bottom_ss_heights),
               units = "bigpts")
    
    # fig3_bottom_ss_grobs[[i]][[j]] <- ss
    fig3_bottom_ss_grobs[[i]][[j]] <- gTree(children = gList(ss, ss_border))
...
# Iterate over bottom panel ss grobs
fig3_bottom_panel_grobs <- vector(mode = "list", length = length(fig3_bottom_ss_grobs))
fig3_bottom_widths_nopadding <- numeric(length(fig3_bottom_ss_grobs))
for (i in 1:length(fig3_bottom_panel_grobs)) {
  ...
  fig3_bottom_panel_widths <- numeric(ncols)
  fig3_bottom_panel_widths[ind_ss] <- fig3_mouse_ss_width_pt
  fig3_bottom_panel_widths[ind_padding] <- fig3_bottom_ss_padding_pt
  
  # Heights of the panel grids
  fig3_bottom_panel_heights <- c(fig3_polar_height_pt, fig3_bottom_ss_padding_pt, sum(fig3_bottom_ss_heights))
  
  # Build the list of grobs for the panel
  fig3_bottom_panel_grobs_list <- vector(mode = "list", length = ngrobs)
  fig3_bottom_panel_grobs_list[1] <- list_polar_plots[i]
  fig3_bottom_panel_grobs_list[2] <- list(zeroGrob())
  fig3_bottom_panel_grobs_list[ind_ss+2] <- fig3_bottom_ss_grobs[[i]]
  fig3_bottom_panel_grobs_list[ind_padding+2] <- list(zeroGrob())
  
  # Layout of the panel grid
  fig3_bottom_panel_layout <-  rbind(rep(1, ncols),
                                     rep(2, ncols),
                                     3:(ncols+2))
  ...
  fig3_bottom_panel_width <- sum(fig3_bottom_panel_widths)
  fig3_bottom_panel_height <- sum(fig3_bottom_panel_heights)  
  
  # Export panel
  outfile <- paste(output_plot_prefix, "bottom_panel_nk", nk_subset[i], sep = "_")
  outfile <- paste0(outfile, ".pdf")
  outfile <- file.path(output_dir, outfile)
  export_pdf(x = fig3_bottom_panel_grobs[[i]],
             file = outfile,
             width = fig3_bottom_panel_width,
             height = fig3_bottom_panel_height,
             units = "bigpts")  
  
  fig3_bottom_widths_nopadding[i] <- fig3_bottom_panel_width
...
fig3_bottom_widths <- numeric(ngrobs)
fig3_bottom_widths[ind_panels] <- fig3_bottom_widths_nopadding
fig3_bottom_widths[ind_padding] <- fig3_bottom_panel_padding_pt

fig3_bottom_heights <- fig3_bottom_panel_height

fig3_bottom_grobs_list <- vector(mode = "list", length = ngrobs)
fig3_bottom_grobs_list[ind_panels] <- fig3_bottom_panel_grobs
fig3_bottom_grobs_list[ind_padding] <- list(zeroGrob())

fig3_bottom_layout <- rbind(1:ngrobs)

fig3_bottom_grob <- arrangeGrob(grobs = fig3_bottom_grobs_list,
                                layout_matrix = fig3_bottom_layout,
                                widths = unit(fig3_bottom_widths, "bigpts"),
                                heights = unit(fig3_bottom_heights, "bigpts"))

fig3_bottom_width <- sum(fig3_bottom_widths)
fig3_bottom_height <- sum(fig3_bottom_heights)

outfile <- paste(output_plot_prefix, "bottom_panel", sep = "_")
outfile <- paste0(outfile, ".pdf")
outfile <- file.path(output_dir, outfile)
export_pdf(x = fig3_bottom_grob,
           file = outfile,
           width = fig3_bottom_width,
           height = fig3_bottom_height,
           units = "bigpts")
```

```{r fig3-build}
# Padding between heatmap and ss
fig3_padding_height_pt <- 20

# Figure 3 dimensions
fig3_widths <- c(fig3_width)
fig3_heights <- c(fig3_bottom_height,
                  fig3_padding_height_pt,
                  fig3_heatmap_height_pt)

# Figure 3 layout
fig3_layout <- rbind(c(01),
                     c(02),
                     c(03))

# Figure 3 grob
fig3_grob <- arrangeGrob(fig3_bottom_grob,
                         zeroGrob(),
                         fig3_heatmap_grob,
...
fig3_heatmap_human_match_legend_vp <- viewport(x = unit(fig3_heatmap_human_match_legend_vp_x, "bigpts"),
                                               y = unit(fig3_heatmap_height_pt+5, "bigpts"),
