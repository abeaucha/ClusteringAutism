...
# Export
outfile <- paste(output_plot_prefix, 5, sep = "_")
...
for (clust in clusters) {
  ...
  img_abs <- import_cluster_map(imgdir = mouse_cluster_map_dirs_50um[["absolute"]],
                                mask = mouse_mask_file,
                                nk = nk, k = k,
                                threshold = threshold,
                                threshold_value = threshold_value,
  ...
  img_rel <- import_cluster_map(imgdir = mouse_cluster_map_dirs_50um[["relative"]],
                                mask = mouse_mask_file,
                                nk = nk, k = k,
                                threshold = threshold,
                                threshold_value = threshold_value,
  ...
  outfile <- paste(output_plot_prefix, 6, clust, clust, sep = "_")
  outfile <- paste0(outfile, ".pdf")
  outfile <- file.path(output_dir, outfile)
...
if (run) {
  for (i in 1:nrow(df_cluster_grid)) {
    ...
    outfile <- paste(output_plot_prefix, 6, id4, id, sep = "_")
    outfile <- paste0(outfile, ".pdf")
    outfile <- file.path(output_dir, outfile)
    export_pdf(x = fig_supp_6_grob,
               file = outfile,
...
if (run) {
  ...
  for (i in 1:nrow(df_sig_gt4)) {
    ...
    # Import centroid image for specific cluster using threshold
    img_abs <- import_cluster_map(imgdir = human_cluster_map_dirs[["absolute"]],
                                  mask = human_mask_file,
                                  nk = nk, k = k,
                                  threshold = threshold,
                                  threshold_value = threshold_value,
    ...
    # Import centroid image for specific cluster using threshold
    img_rel <- import_cluster_map(imgdir = human_cluster_map_dirs[["relative"]],
                                  mask = human_mask_file,
                                  nk = nk, k = k,
                                  threshold = threshold,
                                  threshold_value = threshold_value,
    ...
    outfile <- paste(output_plot_prefix, 6, "mouse", mouse_clust, "human", human_clust, sep = "_")
    outfile <- paste0(outfile, ".pdf")
    outfile <- file.path(output_dir, outfile)
    export_pdf(x = fig_supp_6_grob,
               file = outfile,
...
# Export
outfile <- paste(output_plot_prefix, 9, sep = "_")
outfile <- paste0(outfile, ".pdf")
outfile <- file.path(output_dir, outfile)
pdf(file = outfile,
