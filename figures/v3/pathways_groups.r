...
names(list_cluster_groups) <- c("A (dark green)", "B (orange)", "C (light green)", "D (purple)", "E", "F")
...
list_pathways_groups <- vector(mode = "list", length = length(list_cluster_groups))
...
plt <- ggplot(df_pathways_groups,
       aes(x = cluster_id, 
           y = fct_rev(pathway_trunc), 
           fill = NLQ)) + 
  geom_tile(col = "grey60") +
  facet_grid(pathway_cluster~group,  scales = "free", space = "free", switch = "y") + 
...
outfile <- pdf("heatmap_groups_clustered.pdf",
               width = unit(10, "in"),
               height = unit(10, "in"))
print(plt)
dev.off()
