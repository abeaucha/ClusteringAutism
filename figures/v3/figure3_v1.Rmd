...
```{r fig3-ss-mouse-grob}
# List of mouse slice grobs and zero grobs for padding
...
```{r fig3-ss-human-grob}
# List of human slice grobs and zero grobs for padding
...
## Combine mouse and human slices

```{r fig3-ss-combined}
# Dimensions of ss panel
...
fig3_ss_grob <- arrangeGrob(fig3_human_ss_grob,
                            ...
                            widths = unit(fig3_ss_widths, "bigpts"),
...
```{r fig3-pathways-polar}
# Select cluster solutions
nk_select <- c(4, 6, 8, 10)
df_pathways_subset <- df_pathways_all %>% 
  filter(nk %in% nk_select) %>% 
...
NLQ_threshold_polar <- 10
df_polar_plot <- df_polar_plot %>% 
  mutate(NLQ = ifelse(NLQ > NLQ_threshold_polar, NLQ_threshold_polar, NLQ),
         nk = factor(nk), k = factor(k))

# Polar plot parameters
nspokes <- length(polar_plot_lvls) - 1
theta_start <- -(2*pi)/nspokes
breaks_start <- 0
breaks_end <- NLQ_threshold_polar
breaks_step <- 2
...
```{r fig3-pathways-polar}
polar_palette <- c(carto_pal(12, "Prism")[2],
...
polar_palette <- c(carto_pal(12, "Prism")[2],
                   ...
                   carto_pal(12, "Safe")[4],
                   carto_pal(12, "Prism")[8],
                   carto_pal(12, "Prism")[2],
                   carto_pal(12, "Prism")[2],
                   carto_pal(12, "Safe")[4],
                   carto_pal(12, "Prism")[8])

# Generate polar plot
fig3_pathways_polar <- ggplot(df_polar_plot, 
                              aes(x = pathway, 
                                  ymin = 0, ymax = NLQ, 
                                  group = cluster_id, 
                                  fill = cluster_id, 
  ...
  theme(axis.ticks.x = element_blank(),
        ...
        legend.text = element_text(size = font_size,
                                   family = font_family),
        plot.margin = margin(t = 5, unit = "bigpts")
  )

# Extract grob
fig3_pathways_polar_grob <- grid.force(ggplotGrob(fig3_pathways_polar))

# Fix axis labels in each panel
fig3_pathways_polar_panel_ids <- str_subset(names(fig3_pathways_polar_grob[["children"]]), "panel")
for (panel in fig3_pathways_polar_panel_ids) {
  fig3_pathways_polar_grob[["children"]][[panel]][["children"]][[6]][["children"]][[1]][["children"]][[1]][["label"]] <- pathway_label_lvls_clustered
}
...
fig3_polar_height <- fig3_width/3
...
# Figure 3 dimensions
fig3_widths <- fig3_width*c(0.05, 0.75, 0.12, 0.08)
fig3_heights <- c(fig3_ss_height_pt, fig3_height_padding_pt, fig3_height_padding_pt, fig3_heatmap_height_pt, fig3_height_padding_pt, fig3_polar_height)

# Figure 3 layout
fig3_layout <- rbind(c(01, 01, 01, 01),
                     c(02, 02, 02, 02),
                     c(03, 04, 05, 06),
                     c(07, 04, 04, 04),
                     c(08, 04, 04, 04),
...
# Figure 3 grob
fig3_grob <- arrangeGrob(fig3_ss_grob,
                         # rect_w_num(2),
                         zeroGrob(),
                         # rect_w_num(3),
                         zeroGrob(),
...
# Viewport for heatmap legend
fig3_heatmap_pathways_legend_vp <- viewport(x = unit(fig3_width_pt-10, "bigpts"),
                                            y = unit(fig3_polar_height, "bigpts"),
