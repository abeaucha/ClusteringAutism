...
for (i in 1:nrow(df_kw_clusters)) {
  ...
  anova_out <- anova(lm(y ~ k, data = df_model))
  
  ss_between <- anova_out["k", "Sum Sq"]
  ss_total <- sum(anova_out[, "Sum Sq"])
  
  H <- kw_out[["statistic"]]
  n <- nrow(df_model)
  k <- length(levels(df_model$k))
  
  df_kw_clusters[[i, "H"]] <- H
  df_kw_clusters[[i, "p"]] <- kw_out[["p.value"]]
  df_kw_clusters[[i, "eta2H"]] <- H/(n-1)
  df_kw_clusters[[i, "eta2"]] <- ss_between/ss_total
...
df_kw_clusters <- df_kw_clusters %>% 
  mutate(var_explained = eta2*100)

df_kw_clusters_pond <- df_kw_clusters
```
...
H_max <- 26
var_explained_max <- 7
df_kw_plt_pond <- df_kw_clusters_pond %>% 
  mutate(H_threshold = ifelse(p < p_threshold, H, NA),
         var_explained_threshold = ifelse(p < p_threshold, var_explained, NA),
         significant = ifelse(p < p_threshold, "Yes", "No"),
         significant = factor(significant, levels = c("Yes", "No")))

plt_pond_univariate <- ggplot(df_kw_plt_pond, aes(x = factor(nk), y = fct_rev(label), 
                                             col = var_explained, fill = var_explained_threshold)) +
  geom_point(shape = 21, stroke = 1.0, size = 1.5) +
  scale_fill_gradientn(colors = palette_univariate, limits = c(0, var_explained_max), na.value = "white", guide = "none") +
  scale_colour_gradientn(colors = palette_univariate,
                         limits = c(0, var_explained_max),
                         breaks = seq(0, var_explained_max, by = 1),
  ...
  theme(panel.grid.major = element_blank(),
        ...
        axis.text = element_text(size = 6),
        legend.text = element_text(size = 6, margin = margin(l = 2)),
        legend.title = element_text(size = 6, angle = 270, margin = margin(l = 3)),
        legend.margin = margin(),
        legend.box.margin = margin(),
...
df_kw_clusters_hbn <- df_kw_clusters
```
...
H_max <- 26
var_explained_max <- 7
df_kw_plt_hbn <- df_kw_clusters_hbn %>% 
  mutate(H_threshold = ifelse(p < p_threshold, H, NA),
         var_explained_threshold = ifelse(p < p_threshold, var_explained, NA),
         significant = ifelse(p < p_threshold, "Yes", "No"),
         significant = factor(significant, levels = c("Yes", "No")))

plt_hbn_univariate <- ggplot(df_kw_plt_hbn, aes(x = factor(nk), y = fct_rev(label), 
                                             col = var_explained, fill = var_explained_threshold)) +
  geom_point(shape = 21, stroke = 1.0, size = 1.5) +
  scale_fill_gradientn(colors = palette_univariate, limits = c(0, var_explained_max), na.value = "white", guide = "none") +
  scale_colour_gradientn(colors = palette_univariate, limits = c(0, var_explained_max)) +
