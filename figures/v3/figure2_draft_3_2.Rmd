...
```{r}
output_dir <- file.path("outputs", "figure2_draft_3_2")
if (!(dir.exists(output_dir))) {dir.create(path = output_dir, recursive = TRUE)}
outfile_prefix <- "figure2_draft_3_2"
```
...
df_cluster_coords_human <- df_cluster_coords %>% 
  select(cluster_id, nk, k, x, highlight) %>% 
...
df_segments <- df_matches %>% 
  ...
  left_join(df_cluster_coords_human %>% 
              select(cluster_id, x, y, highlight_human = highlight),
            by = c("human_cluster_id" = "cluster_id")) %>% 
  left_join(df_cluster_coords_mouse %>% 
              select(cluster_id, xend = x, yend = y,  highlight_mouse = highlight),
            by = c("mouse_cluster_id" = "cluster_id")) %>% 
  mutate(highlight = highlight_human | highlight_mouse)
...
plt <- ggplot(df_segments, aes(x = x, y = y)) + 
  geom_segment(aes(xend = xend, yend = yend, alpha = highlight)) +
