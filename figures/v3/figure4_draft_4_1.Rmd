...
```{r outputs}
outfile_prefix <- "figure4_draft_4_1"
...
panel_titles <- c("POND+", "HBN")
panel_titles <- paste0(panel_titles, " (K = ", nki, ")")


list_panel_grobs <- vector(mode = "list", length = length(human_datasets))
names(list_panel_grobs) <- human_datasets
for (i in 1:length(list_panel_grobs)) {
# i <- 1
  ...
  plt_mv_scores <- ggplot(df_mv_scores, aes(x = y, y = LD1, fill = y_pred_prob)) +
    ...
    #                      limits  = c(0, 1),
    #                      breaks = c(0, 0.5, 1.0),
    #                      guide = guide_colourbar(barwidth = unit(20, "bigpts"),
    ...
    theme(legend.margin = margin(),
          ...
          axis.text.x = element_text(size = 6, colour = c("darkorchid1", "seagreen2"), hjust = c(1, 0)),
          plot.margin = margin(l = 0, r = 0, t = 5.5, b = 0))


plt_mv_loadings_common <- ggplot(df_mv_loadings_common, aes(x = acronym, y = LD1, col = group)) +
    geom_hline(yintercept = 0) +
    geom_segment(mapping = aes(yend = 0, xend = acronym)) +
    geom_point(size = 0.9) +
    coord_cartesian(ylim = c(-0.5, 0.5)) +
    scale_y_continuous(breaks = seq(-1, 1, by = 0.25)) +
    scale_colour_manual(values = palette_binary) +
    labs(y = "Linear discriminant loadings",
         x = "Clinical variable") +
    theme_bw() +
...
plt_mv_loadings <- ggplot(df_mv_loadings, aes(x = acronym, y = LD1, col = group)) +
    geom_hline(yintercept = 0) +
    geom_segment(mapping = aes(yend = 0, xend = acronym)) +
    geom_point(size = 0.9) +
    coord_cartesian(ylim = c(-0.5, 0.5)) +
...
plt_loadings_patchwork <- plt_mv_loadings_common + plt_mv_loadings +
  plot_layout(widths = c(0.22, 0.78)) &
  theme(plot.margin = margin(l = 0, r = 2, t = 3, b = 5))
...
plt_mv_roc <- ggplot(df_lda_roc, aes(x = fpr, y = tpr)) +
  ...
  panel_title_grob <- gTree(children = gList(textGrob(panel_titles[i], rot = 90, gp = gpar(fontsize = 8))))
  
  # plt_mv_title_grob <- gTree(children = gList(rectGrob(gp = gpar(fill = "grey85")),
  #                                             textGrob(label = panel_titles[i], 
  #                                                      gp = gpar(fontsize = 9))))
  

panel_widths <- c(10, 130, 5, 135, 5, 235)
  panel_heights <- c(75, 5, 75)
  panel_layout <- rbind(c(01, 02, 03, 04, 07, 08),
                        c(01, 02, 03, 05, 07, 08),
                        c(01, 02, 03, 06, 07, 08))

  panel_grob <- arrangeGrob(panel_title_grob,
                            rectGrob(),
                            rectGrob(),
                            ggplotGrob(plt_mv_scores),
                            rectGrob(),
                            plt_mv_roc_grob,
                            zeroGrob(),
                            patchworkGrob(plt_loadings_patchwork),
                            layout_matrix = panel_layout,
                            widths = unit(panel_widths, "bigpts"),
                            heights = unit(panel_heights, "bigpts"))

  
  panel_height_pt <- sum(panel_heights)
  panel_width_pt <- fig_width
  
  panel_height_in <- panel_height_pt/72
  panel_width_in <- panel_width_pt/72
  
  outfile <- paste0("panel_", nki, "_", human_datasets[i], ".pdf")
  outfile <- file.path(output_dir, outfile)
  pdf(file = outfile,
...
fig_heights <- c(panel_height_pt, 10, panel_height_pt)
fig_layout <- cbind(1:3)
...
pushViewport(viewport(x = unit(457, "bigpts"),
                      y = unit(670, "bigpts"),
...
grid.text(label = c("a", "b", "c", "d"),
          x = unit(6, "bigpts"),
          y = unit(fig_height_pt - c(9, 175, 325, 435), "bigpts"),
