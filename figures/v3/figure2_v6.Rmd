...
```{r}
df_similarity_nk %>% 
  select(human_cluster_id, mouse_cluster_id, similarity, pval)
```
...
```{r}
df_mouse_pathways %>% 
  mutate(pathway = factor(pathway, pathway_lvls_clustered)) %>% 
  filter(cluster_id == "4-1") %>% 
  select(pathway, E, adj.P.Val, NLQ) %>% 
  filter(NLQ > 1.3) %>% 
  arrange(pathway)
```
