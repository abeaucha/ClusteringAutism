...
```{r}
output_dir <- file.path("outputs", "figure2_draft_3")
outfile_prefix <- "figure2_draft_3"
```
...
plt_mouse_sankey <- ggplot(df_mouse_sankey,
  ...
  scale_x_discrete(position = "top", expand = expansion(add = c(stratum_width/2+0.020, stratum_width/2+0.005))) +
  # scale_x_discrete(position = "top", expand = expansion(mult = c(0.0, 0.0))) +
  scale_y_continuous(breaks = c(seq(0, 130, by = 20), nrow(mouse_clusters))) + 
  labs(x = "K",
       y = "Number of models") + 
  # theme_void() + 
  theme_bw() +
...
list_mouse_slc_vps <- list(
  "2" = list(x = rep(0.15, 2),
             ...
             h = rep(0.1, 2)),
  "6" = list(x = rep(0.55, 6),
             y = seq(0.1, 0.8, length.out = 6),
             w = rep(0.1, 6),
             h = rep(0.1, 6)),
  "10" = list(x = rep(0.9, 10),
...
```{r}
list_mouse_slices <- vector(mode = "list", length = length(nk_seq))
names(list_mouse_slices) <- nk_seq
...
plt_human_sankey <- ggplot(df_human_sankey,
  ...
  scale_y_continuous(breaks = c(seq(0, 600, by = 100), nrow(human_clusters))) + 
  labs(x = "K",
...
```{r}
list_human_slices <- vector(mode = "list", length = length(nk_seq))
names(list_human_slices) <- nk_seq
