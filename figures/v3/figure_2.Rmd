...
# Convert human clusters to long format
...
```{r cluster-groups}
# Mouse clusters corresponding to different groups/streams
...
# Import cross-species similarity data
...
# Identify mouse clusters with matches in HBN
has_match_hbn <- list_similarity[[2]] %>% 
  filter(match) %>% 
  pull(mouse_cluster_id) %>% 
  unique()

# Add cross-species match information to cluster groups
...
# Filter cluster groups for those with matches
...
```{r sankey-prep}
# Subset of cluster solutions to include
nk_subset <- c(2, 6, 8)

# Convert mouse cluster information to long format
...
# Filter clusters for subset
df_mouse_sankey <- df_mouse_clusters_long %>%
  filter(nk %in% nk_subset) 

# Relabel mouse clusters with custom cluster labels
...
# Create palette for Sankey diagram
df_palette <- df_mouse_cluster_labels %>% 
  filter(nk_old %in% nk_subset) %>% 
  left_join(df_cluster_groups_matches %>% 
              select(cluster_id = mouse_cluster_id, group),
            by = "cluster_id") %>% 
  mutate(colour = ifelse(is.na(group), "grey80", colour)) %>% 
...
plt_mouse_sankey <- ggplot(df_mouse_sankey,
                           aes(x = nk,
                               ...
                               fill = cluster_id,
                               label = k)) + 
  geom_flow(stat = "alluvium", aes.flow = "backward", 
            width = stratum_width, col = "grey30", linewidth = 0.15) + 
...
sankey_widths <- c(sankey_border_width_l, sankey_label_width, 
                   sankey_padding_width, sankey_label_width, 
                   sankey_padding_width, sankey_label_width, 
                   sankey_border_width_r)
...
# Import slices for different K-k
...
for (i in 1:length(list_mouse_slices)) {
  ...
  for (k in 1:nki) {
    ...
    # Get initial cluster K
    nk_init <- df_mouse_cluster_labels %>% 
      filter(nk_new == nki, k_new == k) %>% 
      pull(nk_old)
    
    # Get initial cluster k
    k_init <- df_mouse_cluster_labels %>% 
      filter(nk_new == nki, k_new == k) %>% 
      pull(k_old)
...
human_slc <- list(
  "2" = c(66, 66),
  "6" = c(66, 66, 66, 66, 66, 66),
  "8" = c(66, 66, 66, 66, 66, 66, 66, 66)
)

```

```{r mouse-human-matches}
# List of mouse-human matches
list_matches <- list(
...
for (i in 1:length(list_human_slices_POND)) {
  ...
  for (k in 1:nki) {
    ...
    cluster_id <- paste0(nk_init, "-", k_init)
    
    # If mouse cluster has POND match, import POND slice
    if (cluster_id %in% list_matches[["POND"]]$mouse_cluster_id) {
      
      # Matching POND cluster
      ...
      # If no POND match, generate empty square      
...
for (i in 1:length(list_human_slices_HBN)) {
  ...
  for (k in 1:nki) {
    ...
    # If mouse cluster has HBN match, import HBN slice
    if (cluster_id %in% list_matches[["HBN"]]$mouse_cluster_id) {
      
      # Matching HBN cluster
      ...
      # If no HBN match, generate empty square
...
```{r neuro-ss-grobs}
# Get full height of Sankey panel
sankey_height <- sum(sankey_heights)

# Parameters for slice series
# Slice widths, and padding between mouse and human slices
list_ss_params <- list(
  # K = 2
  list(ss_slc_w = 44,
       ss_pad_w = 4),
...
# Neuroanatomy viewport parameters
list_vp_params <- list(
  # K = 2
  list(x = 115,
  ...
  # K = 6
  list(x = 324,
       y = sankey_height - c(102, 185, 270, 313, 360, 430)),
  # K = 8
  list(x = 533,
       y = sankey_height - c(102, 150, 210, 260, 303, 346, 395, 447))
)

# Neuroanatomy label grobs
...
# Construct viewports for neuro slices
...
# Layout for neuro slice series
ss_layout <- rbind(1:5)

# Construct grobs for each slice series
...
# Figure base grob (Sankey diagram)
...
# Combine Sankey grob and neuro slices
