...
```{r}
output_dir <- file.path("outputs", "figure2_draft_5_1")
if (!(dir.exists(output_dir))) {dir.create(path = output_dir, recursive = TRUE)}
outfile_prefix <- "figure2_draft_5_1"
```
...
```{r}
list_matches <- list(
  "POND" = tibble(mouse_cluster_id = c("2-1", "2-2", "4-1", "4-4", "8-1", "8-2", "8-5", "8-6"),
                          human_cluster_id = c("3-3", "2-1", "4-1", "4-2", "9-5", "8-7", "8-4", "8-2")),
  "HBN" = tibble(mouse_cluster_id = c("2-1", "2-2", "4-1", "4-3", "4-4", "8-1", "8-2", "8-3", "8-4", "8-5", "8-6"),
...
# dx_padding_width <- 10
# dx_panel_tot_width <- fig_width - 2*dx_padding_width
# dx_panel_width <- dx_panel_tot_width/9
# 
# dx_widths <- c(dx_panel_width, dx_panel_width, dx_padding_width, dx_panel_width, dx_panel_width, dx_panel_width, dx_padding_width, dx_panel_width, dx_panel_width, dx_panel_width, dx_panel_width)
# 
# tmp <- arrangeGrob(list_dx_grobs$POND[["2"]][[1]],
#                    list_dx_grobs$POND[["2"]][[2]],
#                    rectGrob(), 
#                    list_dx_grobs$POND[["4"]][[1]],
#                    list_dx_grobs$POND[["4"]][[2]],
#                    list_dx_grobs$POND[["4"]][[3]],
#                    rectGrob(), 
#                    list_dx_grobs$POND[["8"]][[1]],
#                    list_dx_grobs$POND[["8"]][[2]],
#                    list_dx_grobs$POND[["8"]][[3]],
#                    list_dx_grobs$POND[["8"]][[4]],
#                    layout_matrix = rbind(1:11),
#                    widths = unit(dx_widths, "bigpts"))
```
...
list_vp_params <- list(
  ...
  # K = 4
  list(x = 324,
       y = fig_height - c(120, 215, 268, 375)),
  # K = 8
  list(x = 533,
       # y = fig_height - (90 + 50*0:7)
       y = fig_height - c(95, 146, 200, 255, 297, 340, 390, 445))
)
...
fig_height_in <- sum(fig_base_heights)/72
