...
```{r}
comparison <- sort(c("chromatin", "synaptic"))
palette_binary <- c(
  "synaptic" = "seagreen2",
  "chromatin" = "darkorchid1"
  # "mapkwnt" = "springgreen4"
  )
...
```{r}
tmp <- list_clusters$POND %>% 
  select(ID, nk7, DX = dx) %>% 
  left_join(list_diagnoses$POND, by = "DX") %>% 
  mutate(cluster_id = paste0("7-", nk7)) %>% 
  filter(cluster_id %in% list_clusters_compare$POND)

chisq.test(x = tmp$DX_new, y = tmp$cluster_id, simulate.p.value = TRUE)
```

```{r}
tmp <- list_clusters$HBN %>% 
  select(ID, nk7, DX = dx) %>% 
  left_join(list_diagnoses$HBN, by = "DX") %>% 
  mutate(cluster_id = paste0("7-", nk7)) %>% 
  filter(cluster_id %in% list_clusters_compare$HBN)
...
fig_grob <- arrangeGrob(ggplotGrob(plt_diagnoses),
                        zeroGrob(), #Pad
