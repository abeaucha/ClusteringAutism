...
```{r outputs}
outfile_prefix <- "figure4_draft_3_1"
...
# i <- 1
  ...
  plt_mv_loadings_common <- ggplot(df_mv_loadings_common, aes(x = LD1, y = acronym, col = group)) +
    geom_vline(xintercept = 0) + 
    geom_segment(mapping = aes(xend = 0, yend = acronym)) + 
    geom_point() +
  ...
  plt_loadings_common_height <- 100
  ...
  panel_heights <- c(panel_title_height, 
                     ...
                     plt_roc_height,
                     panel_padding_height,
                     plt_loadings_common_height,
                     panel_padding_height,
                     plt_loadings_height)
  panel_layout <- cbind(1:8)
  ...
  outfile <- paste0("panel_", nki, human_datasets[i], ".pdf")
  outfile <- file.path(output_dir, outfile)
  pdf(file = outfile,
...
fig_layout <- rbind(1:4)

padding_width <- 15
legend_width <- 55
panel_width <- (fig_width - padding_width - legend_width)/2
