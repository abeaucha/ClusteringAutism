...
```{r}
df_enrichment %>% 
  filter(Root == "Signal Transduction", Parent == "Signal Transduction") 
```
...
```{r}
i <- 9
root_name <- df_reactome_roots[[i, "Name"]]
root_name <- "Disease"

df_enrichment %>% 
  group_by(k) %>% 
  top_n(n = -20, wt = rank) %>% 
  filter(Root == root_name) %>% 
  select(k, Title, level, Parent, Root, B) %>% 
  arrange(k, level) %>% 
  filter(level == 4)
```
...
df_top_n_root %>% 
  mutate(n = ifelse(is.na(n), 0, n),
         enriched = n > 0) %>% 
  group_by(Root) %>% 
  summarise(ncluster = sum(enriched)) %>% 
  arrange(desc(ncluster))
```
...
df_enrichment %>% 
  filter(adj.P.Val < 0.01, 
         Root == "Signal Transduction") %>% 
  select(Root, k, Title, level, Parent, B, NLQ) %>% 
  arrange(k, level) %>% 
  filter(level == 2)
```
