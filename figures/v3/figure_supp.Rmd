...
```{r fig-supp-8-11-ss}
list_mouse_clusters <- list(c("4-1", "6-5", "8-6", "10-4"),
                            c("4-4", "6-1", "8-1"),
                            c("4-4", "6-4", "8-2", "10-5"),
                            c("4-3", "8-5", "10-3"))
...
for (l in 1:length(list_mouse_clusters)) {

  # Iterate over clusters
  clusters <- list_mouse_clusters[[l]]
  for (mouse_clust in clusters) {
    
    print(paste("Mouse cluster:", mouse_clust))
    ...
    human_matches <- df_sim_pvals %>% 
      filter(mouse_cluster_id == mouse_clust, significant == 1) %>% 
      pull(human_cluster_id)
    
    for (human_clust in human_matches) {
      
      print(paste("Human cluster:", human_clust))
      
      # Extract cluster IDs
      nk_human <- str_split(mouse_clust, pattern = "-", simplify = TRUE)[1]
      k_human <- str_split(mouse_clust, pattern = "-", simplify = TRUE)[2]
...
for (i in 1:length(list_plots)) {
  ...
  axis_params <- df_plsda_summary %>% 
    ...
    mutate(fontface = ifelse(matches == "Both", "bold", "plain"),
           ...
           fontcolour_1 = ifelse(has_match_1, "black", "grey30"),
           fontcolour_2 = ifelse(has_match_2, "black", "grey30"),
...
legend_labels_1 <- character(length(plsda_comparisons))
legend_labels_2 <- character(length(plsda_comparisons))
for (i in 1:length(plsda_comparisons)) {
  legend_labels_1[i] <- plsda_comparisons[[i]][1]
  legend_labels_2[i] <- plsda_comparisons[[i]][2]
  
}

legend_labels_1 <- str_flatten(legend_labels_1, collapse = ", ")
legend_labels_2 <- str_flatten(legend_labels_2, collapse = ", ")

legend_labels <- c(legend_labels_1, legend_labels_2)
```
...
list_xlims <- list(c(-4.5, 3.5),
                   c(-4, 4),
                   c(-4.5, 4.5),
                   c(-4.5, 4.5))
list_ylims <- list(c(-3, 4),
                   c(-3, 3.5),
                   c(-3.5, 4),
                   c(-3.5, 4))
...
# p_plsda_loadings <- (list_plots_histx[[1]] | plot_spacer() | list_plots_histx[[2]] | plot_spacer()) / 
#   (list_plots_scatter[[1]] | list_plots_histy[[1]] | list_plots_scatter[[2]] | list_plots_histy[[2]]) / 
#   (list_plots_histx[[3]] | plot_spacer() | list_plots_histx[[4]] | plot_spacer()) / 
#   (list_plots_scatter[[3]] | list_plots_histy[[3]] | list_plots_scatter[[4]] | list_plots_histy[[4]]) +
#   plot_layout(widths = c(3,1,3,1))

p_plsda_loadings <- ((list_patchworks[[1]]) | (list_patchworks[[2]])) / (list_patchworks[[3]] | list_patchworks[[4]]) 
...
outfile <- "plsda_loadings.pdf"
outfile <- file.path(output_dir, outfile)
pdf(file = outfile,
    width = unit(width_in, "in"),
    height = unit(height_in, "in"))
print(p_plsda_loadings)
dev.off()
```
...
params_marginals <- list(position = "dodge",
                         binwidth = 0.5)

list_plots <- vector(mode = "list", length = length(list_scores)) 
for (i in 1:length(list_plots)) {
  
  df_scores <- list_scores[[i]] %>% 
    # mutate(cluster = ifelse(cluster == plsda_comparisons[[i]][1], "k1", "k2"))
    mutate(cluster = factor(cluster, levels = plsda_comparisons[[i]]))
  df_loadings <- list_loadings[[i]]
  
  p_i <- ggplot(df_scores, mapping = aes(x = x, y = y, col = cluster)) + 
    geom_point() + 
    annotate("segment", 
             x = 0, y = 0, 
             xend = df_loadings$x, 
  ...
  list_plots[[i]] <- patchwork::wrap_elements(ggMarginal(p_i, type = "histogram", 
                                                         groupColour = TRUE, 
                                                         groupFill = TRUE, 
                                                         xparams = params_marginals,
                                                         yparams = params_marginals))
}

p_plsda_loadings <- ((list_plots[[1]] + list_plots[[2]]) / (list_plots[[3]] + list_plots[[4]]))
...
(list_plots[[1]] + list_plots[[2]]) / (list_plots[[3]] + list_plots[[4]])
```
