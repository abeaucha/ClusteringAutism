...
```{r}
outfile_prefix <- "figure3_draft_2_1"
...
```{r}
plt_human_variants_sankey <- ggplot(tmp, 
       aes(x = nk, stratum = k,
           alluvium = ID)) + 
  geom_flow(mapping = aes(fill = variant_identified, col = variant_identified, linetype = variant_identified, linewidth = variant_identified),
            stat = "alluvium", aes.flow = "forward",
            alpha = 1) +
  scale_colour_manual(values = c("grey80", "springgreen4"), guide = "none") + 
  scale_fill_manual(values = c("grey80", "springgreen4"), guide = "none") + 
  ...
  new_scale_colour() +
  geom_stratum(mapping = aes(fill = n_variants),
               width = 0.4, linewidth = 0.15) +
  scale_fill_manual(values = variants_palette_tmp,
                    guide = guide_legend(title.position = "bottom", title.hjust = 0.5)) + 
  # scale_colour_manual(values = c("grey50", "springgreen4"), guide = "none") +
...
human_variants_sankey_legend <- plt_human_variants_sankey %>% 
    ggplotGrob() %>%
  grid.force() %>%
  getGrob("guides.3-3-3-3")

plt_human_variants_sankey <- plt_human_variants_sankey + 
  theme(legend.position = "none")

# sankey_grobs <- grid.force(ggplotGrob(plt_human_variants_sankey))
# grid.ls(sankey_grobs)
...
human_variants_widths <- c(258, 150, 204)
human_variants_height <- 200

human_variants_layout <- rbind(1:3)

human_variants_grob <- arrangeGrob(ggplotGrob(plt_human_variants_sankey),
                                   ...
                                   heights = unit(human_variants_height, "bigpts"))


# plt_human_variants_patchwork <- plt_human_variants_sankey | plt_human_variants_heatmap | plt_human_pathway_metrics +
#   plot_layout(widths = c(0.6, 0.2, 0.2)) 
#   # theme(plot.margin = margin(l = 0))
# human_variants_patchwork_grob <- patchworkGrob(plt_human_variants_patchwork)
```
