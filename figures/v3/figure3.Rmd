...
```{r mouse-meta-clusters}
# CODE TO GENERATE CLUSTER META-CLUSTERS FROM ENRICHMENT PATTERNS
...
```{r mouse-heatmap-dendrogram-col}
# CODE TO EXTRACT COLUMN DENDROGRAM FOR META CLUSTERS
...
# cluster_lvls <- df_pathways_subset %>% 
...
```{r mouse-pathways-heatmap-patchwork}
# Combine main heatmap and cluster groups
...
```{r}
# file_palette <- "human_cluster_colours_POND.xlsx"
...
df_human_sankey_annotations$y <- c(c(0.78, 0.35), #2
                                   ...
                                   c(0.78, 0.62, 0.57, 0.54, 0.45, 0.36, 0.33, 0.25), #8
                                   c(0.78, 0.63, 0.57, 0.54, 0.51, 0.45, 0.365, 0.20, 0.25), #9
                                   c(0.78, 0.67, 0.62, 0.57, 0.54, 0.51, 0.25, 0.23, 0.22, 0.25)) #10
...
## POND+ molecular pathways heatmap
...
human_clusters_variants_new_labs <- df_human_cluster_labels %>% 
  filter(cluster_id %in% human_clusters_variants) %>% 
  pull(cluster_id_new)
```
...
plt_human_variants_heatmap <- ggplot(df_human_heatmap, 
  ...
  scale_x_discrete(expand = expansion(), labels = human_clusters_variants_new_labs) + 
  scale_y_discrete(expand = expansion(), position = "right") +
  scale_fill_gradientn(colors = heatmap_palette,
                       limits = heatmap_limits,
...
## Mouse-POND+ molecular pathway correspondence
...
human_pathway_metrics_labs <- df_human_variant_matches %>% 
  left_join(df_human_cluster_labels, by = c("human_cluster_id" = "cluster_id")) %>% 
  pull(cluster_id_new)

human_pathway_metrics_labs <- paste0("  ", human_pathway_metrics_labs)
```
...
fig_heights <- c(20, polar_height_pt, panel_padding_height, heatmap_height, 10, human_variants_height)
...
pushViewport(viewport(x = unit(540, "bigpts"),
                      y = unit(32, "bigpts"),
                      width = unit(140, "bigpts"),
                      height = unit(34, "bigpts")))
# grid.rect(gp = gpar(fill = "grey70"))
...
grid.text(label = c("A", "B", "C", "D"),
          x = unit(c(10.0, 10.0, 298, 468), "bigpts"),
          y = unit(fig_height_pt - c(8, 517, 517, 517), "bigpts"),
