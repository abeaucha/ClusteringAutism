...
Conners - C3SR
- Defiance/Aggression T-score: C3SR_AG_T
- Family relations T-score: C3SR_FR_T
- Hyperactivity/Impulsivity T-score: C3SR_HY_T
- Inattention T-score: C3SR_IN_T
- Learning problems t-score: C3SR_LP_T
Conners Adult Self Rating - CAARS
Children's depression index - parent - CDI_P
- Emotional problems T-score: CDI2P_EP_T
- Function problems T-score: CDI2P_FP_T
- Total score: CDI2P_Total
Columbia Impairment scale - CIS_P, CIS_SR
...
plt_sankey <- ggplot(df_clusters_hbn_long,
...
df_clinical_completion_hbn <- df_clusters_clinical_hbn %>% 
  select(subject_id, ACE_Score:CELF_LCI_Stnd) %>% 
...
for (i in 1:length(nk_seq)) {
  ...
  plt_mv_loadings <- ggplot(df_mv_loadings, aes(x = LD1, y = variable, col = group)) +
    geom_vline(xintercept = 0) + 
    # geom_segment(mapping = aes(xend = 0, yend = acronym)) + 
