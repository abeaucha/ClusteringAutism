...
```{r}
outfile_prefix <- "figure3_draft_1_PCA_lvl_1-2_n_30"
...
ggplot(df_pca, aes(x = PC1, y = PC2, col = factor(level))) + 
  geom_point()
```
...
```{r}
ggplot(df_pca, aes(x = PC3, y = PC4, col = factor(level))) + 
  geom_point()
```

```{r}
df_var_explained <- tibble(y = 100*(pca_out$sdev^2)/sum(pca_out$sdev^2),
                           x = 1:length(pca_out$sdev))

ggplot(df_var_explained, aes(x = x, y = y)) + 
  geom_col() +
