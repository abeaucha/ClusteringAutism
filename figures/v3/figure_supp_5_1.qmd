...
plt_human_sankey <- ggplot(
  ...
  scale_y_continuous(breaks = c(seq(0, 850, by = 50), nrow(df_human_clusters)),
...
df_POND_labels <- readxl::read_excel(file.path(PROJECTPATH, "figures/v3/resources/POND_cluster_labels_colours.xlsx"))

df_similarity %>% 
  left_join(df_HBN_labels, by = c("img2_cluster_id" = "cluster_id")) %>% 
  rename(HBN_cluster_id_old = img2_cluster_id, 
         HBN_cluster_id_new = cluster_id_new, 
         HBN_colour = colour) %>% 
  left_join(df_POND_labels, by = c("img1_cluster_id" = "cluster_id")) %>% 
  select(-img1_cluster_id, POND_colour = colour) %>% 
  rename(POND_cluster_id_new = cluster_id_new) %>% 
  select(HBN_cluster_id_old, HBN_cluster_id_new, 
         POND_cluster_id_new, POND_nk = img1_nk, HBN_colour, POND_colour, pval) %>% 
  filter(HBN_cluster_id_new == "7-6", POND_nk == 7) %>% 
  arrange(pval)
...
list_vp_params <- list(
  ...
  # K = 5
  list(x = 216,
       y = fig_height_pt - c(54, 188, 275, 400, 520)),
  # K = 6
  list(x = 274,
       y = fig_height_pt - c(45, 171, 225, 280, 410, 525)),
  # K = 7
  list(x = 329,
       y = fig_height_pt - c(40, 138, 182, 256, 375, 456, 530)),
  # K = 8
  list(x = 386,
       y = fig_height_pt - c(40, 135, 175, 230, 302, 392, 474, 538)),
  # K = 9
  list(x = 443,
       y = fig_height_pt - c(40, 97, 136, 183, 267, 351, 418, 474, 543)),
  # K = 10
  list(x = 500,
       y = fig_height_pt - c(40, 91, 130, 175, 250, 326, 384, 434, 484, 543))
...
df_similarity %>% 
  left_join(df_cluster_labels, by = c("img2_cluster_id" = "cluster_id", 
                                      "img2_nk" = "nk_old", "img2_k" = "k_old")) %>% 
  filter(pval < 0.05) %>% 
  filter(nk_new == 10) %>% 
  select(img1_cluster_id, cluster_id_new)
```
