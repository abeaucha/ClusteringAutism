...
if (dataset == "MICe") {
  ...
  pipeline_dir <- file.path(PROJECTPATH, "data/mouse/derivatives/v3/", params_id)
  
  es_dir <- file.path(pipeline_dir, "effect_sizes", "200")
  cluster_dir <- file.path(pipeline_dir, "clusters", "resolution_0.2")
  
  mask_file <- file.path(PROJECTPATH, "data/mouse/registration/reference_files", "scanbase_second_level-nlin-3_mask_200um.mnc")
  ...
  pipeline_dir <- file.path(PROJECTPATH, "data/human/derivatives/v3/", params_id)
...
# 
if (dataset == "MICe") {
  list_es <- import_effect_sizes(path = es_dir, clusters = clusters, mask = mask_file)
} else {
