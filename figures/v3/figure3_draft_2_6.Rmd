...
```{r}
outfile_prefix <- "figure3_draft_2_6"
...
```{r}
df_cluster_groups <- bind_rows(
tibble(mouse_cluster_id = c("2-1", "3-3", "4-4", "5-1", "6-6", "7-5"),
       group = "A"),
tibble(mouse_cluster_id = c("4-3", "5-3", "6-3", "7-3", "8-5", "9-4", "10-3"),
       group = "B"),
tibble(mouse_cluster_id = c("5-5", "6-4", "7-4", "8-2", "9-5", "10-5"),
       group = "C"),
tibble(mouse_cluster_id = c("6-1", "7-1", "8-1", "9-1", "10-1"),
       group = "D"),
tibble(mouse_cluster_id = c("2-2", "3-1", "4-1", "5-4", "6-5", "7-7", "8-6", "9-2", "10-4"),
       group = "E"))
...
df_cluster_groups <- df_cluster_groups %>% 
  mutate(pond_match = mouse_cluster_id %in% has_match_pond,
         hbn_match = cluster_id %in% has_match_hbn,
...
df_sankey_palette <- df_sankey_palette %>% 
  # select(cluster_id, colour = colour_1) %>% 
  select(cluster_id, colour = colour_2)
