...
outfile <- paste0("figure3_PONDSK_MICe_heatmap_with_legend.pdf")
...
fig3_heatmap_PONDSK_MICe <- fig3_heatmap_PONDSK_MICe +
...
```{r fig3-PONDSK-MICe-nk6-heatmap-similarity}
# Cluster solutions to visualize
nk_1 <- 6
nk_2 <- 6
...
```{r fig3-PONDSK-MICe-nk6-heatmap-permutations}
# Filter permutations data for desired cluster numbers
# and combine Jacobians
df_permutations_nk <- list_permutations %>% 
  bind_rows() %>% 
...
```{r fig3-PONDSK-MICe-nk6-heatmap-pvals}
# Compute p-values for cluster correlations
df_similarity_nk[["pval"]] <- 0
for (i in 1:nrow(df_similarity_nk)) {
  df_similarity_nk[[i, "pval"]] <- sum(df_permutations_nk[["similarity"]] >= df_similarity_nk[[i, "similarity"]])/nrow(df_permutations_nk)
...
```{r fig3-PONDSK-MICe-nk6-heatmap-plot}
# Clamp log p-values when infinite
df_similarity_nk <- df_similarity_nk %>% 
  mutate(pval_log = ifelse(is.infinite(pval_log), 3, pval_log))

# Heatmap plot
fig3_heatmap_PONDSK_MICe_nk6 <- ggplot(df_similarity_nk, 
                                       aes(x = img2_cluster_id, 
                                           y = fct_rev(img1_cluster_id), 
                                           fill = similarity)) + 
  geom_tile(col = "grey50") + 
...
# Export
outfile <- paste0("figure3_PONDSK_MICe_heatmap_nk6.pdf")
outfile <- file.path(output_dir, outfile)
pdf(file = outfile,
    width = unit(500/72, "in"),
    height = unit(500/72, "in"))
print(fig3_heatmap_PONDSK_MICe_nk6)
dev.off()
```

```{r fig3-PONDSK-MICe-nk6-matches}
df_matches_PONDSK_MICe_nk6 <- df_similarity_nk %>% 
  filter(pval_lab != "") %>% 
  select(POND = img1_cluster_id, MICe = img2_cluster_id)
```

```{r fig3-PONDSK-MICe-nk6-export}
outfile <- "PONDSK_MICe_nk6.csv"
outfile <- file.path(output_dir, outfile)
df_similarity_nk %>% 
    select(img1_cluster_id, img2_cluster_id, similarity, pval) %>% 
  write_csv(file = outfile)
...
```{r fig3-HBN-MICe-nk6-heatmap-similarity}
# Cluster solutions to visualize
nk_1 <- 6
nk_2 <- 6
...
```{r fig3-HBN-MICe-nk6-heatmap-permutations}
# Filter permutations data for desired cluster numbers
# and combine Jacobians
df_permutations_nk <- list_permutations %>% 
  bind_rows() %>% 
...
```{r fig3-HBN-MICe-nk6-heatmap-pvals}
# Compute p-values for cluster correlations
df_similarity_nk[["pval"]] <- 0
for (i in 1:nrow(df_similarity_nk)) {
  df_similarity_nk[[i, "pval"]] <- sum(df_permutations_nk[["similarity"]] >= df_similarity_nk[[i, "similarity"]])/nrow(df_permutations_nk)
...
```{r fig3-HBN-MICe-nk6-heatmap-plot}
# Clamp log p-values when infinite
df_similarity_nk <- df_similarity_nk %>% 
  mutate(pval_log = ifelse(is.infinite(pval_log), 3, pval_log))

# Heatmap plot
fig3_heatmap_HBN_MICe_nk6 <- ggplot(df_similarity_nk, 
                                    aes(x = img2_cluster_id, 
                                        y = fct_rev(img1_cluster_id), 
                                        fill = similarity)) + 
  geom_tile(col = "grey50") + 
...
# Export
outfile <- paste0("figure3_HBN_MICe_heatmap_nk6.pdf")
outfile <- file.path(output_dir, outfile)
pdf(file = outfile,
    width = unit(500/72, "in"),
    height = unit(500/72, "in"))
print(fig3_heatmap_HBN_MICe_nk6)
dev.off()
```

```{r fig3-HBN-MICe-nk6-matches}
df_matches_HBN_MICe_nk6 <- df_similarity_nk %>% 
  filter(pval_lab != "") %>% 
  select(HBN = img1_cluster_id, MICe = img2_cluster_id)
```

```{r fig3-HBN-MICe-nk6-export}
outfile <- "HBN_MICe_nk6.csv"
outfile <- file.path(output_dir, outfile)
df_similarity_nk %>% 
    select(img1_cluster_id, img2_cluster_id, similarity, pval) %>% 
  write_csv(file = outfile)
...
```{r fig3-PONDSK-HBN-nk6-similarity}
# Cluster solutions to visualize
nk_1 <- 6
nk_2 <- 6
...
```{r fig3-PONDSK-HBN-nk6-permutations}
# Filter permutations data for desired cluster numbers
# and combine Jacobians
df_permutations_nk <- list_permutations %>% 
  bind_rows() %>% 
...
```{r fig3-PONDSK-HBN-nk6-plot}
# Clamp log p-values when infinite
df_similarity_nk <- df_similarity_nk %>% 
  mutate(pval_log = ifelse(is.infinite(pval_log), 3, pval_log))

# Heatmap plot
fig3_heatmap_PONDSK_HBN_nk6 <- ggplot(df_similarity_nk, 
                                      aes(x = img2_cluster_id, 
                                          y = fct_rev(img1_cluster_id), 
                                          fill = similarity)) + 
  geom_tile(col = "grey50") + 
...
# Export
outfile <- paste0("figure3_PONDSK_HBN_heatmap_nk6.pdf")
outfile <- file.path(output_dir, outfile)
pdf(file = outfile,
    width = unit(500/72, "in"),
    height = unit(500/72, "in"))
print(fig3_heatmap_PONDSK_HBN_nk6)
dev.off()
```

```{r fig3-PONDSK-HBN-nk6-matches}
df_matches_PONDSK_HBN_nk6 <- df_similarity_nk %>% 
  filter(pval_lab != "") %>% 
  select(POND = img1_cluster_id, HBN = img2_cluster_id)
```

```{r fig3-PONDSK-HBN-nk6-export}
outfile <- "PONDSK_HBN_nk6.csv"
outfile <- file.path(output_dir, outfile)
df_similarity_nk %>% 
    select(img1_cluster_id, img2_cluster_id, similarity, pval) %>% 
  write_csv(file = outfile)
...
fig3_heatmap <- (fig3_heatmap_PONDSK_MICe | fig3_heatmap_HBN_MICe | fig3_heatmap_PONDSK_HBN) /
  (fig3_heatmap_PONDSK_MICe_nk6 | fig3_heatmap_HBN_MICe_nk6 | fig3_heatmap_PONDSK_HBN_nk6) +
...
# Combine data set matches for 6-cluster solutions
df_matches_nk6 <- df_matches_PONDSK_MICe_nk6 %>% 
  inner_join(df_matches_HBN_MICe_nk6, by = "MICe")

# Combine data set matches
df_matches <- bind_rows(df_matches_opt, df_matches_nk6)
...
# Relative centroid directory
centroid_dir <- file.path(PROJECTPATH, "data/human/derivatives", 
                          version, params_id, 
...
# Relative centroid directory
centroid_dir <- file.path(PROJECTPATH, "data/mouse/derivatives", 
                          version, params_id, 
                          "centroids/resolution_0.05/relative/")
