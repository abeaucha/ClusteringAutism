...
fig4_total_width <- 510
fig4_total_height <- 481

fig4_border_padding_width <- 4
fig4_border_padding_height <- fig4_border_padding_width

fig4_width <- fig4_total_width - 2*fig4_border_padding_width
```
...
```{r fig4-similarity-import-permutations}
...
```{r fig4-similarity-compute-pvals}
...
```{r fig4-heatmap-pathways-import}
# Database versions
...
```{r fig4-heatmap-pathway-clust-proc}
...
```{r fig4-heatmap-pathway-clust-scree-plot}
# Generate pathway cluster scree plot
fig4_pathway_scree_plot <- mat_pathways_nk_gt4_subset %>% 
  t() %>% 
  hclust_wcss() %>% 
  as_tibble() %>% 
  mutate(nk = 1:nrow(.)) %>% 
...
pdf(file = outfile,
    width = unit(8, "in"),
    height = unit(4, "in"))
print(fig4_pathway_scree_plot)
dev.off()
```

```{r fig4-heatmap-pathway-clust-labels}
...
```{r fig4-heatmap-dendrogram-plot}
# Generate clustered heatmap
fig4_pheatmap <- pheatmap(mat = mat_pathways_nk_gt4_subset,
                          cluster_col = FALSE,
                          cluster_rows = TRUE,
...
fig4_heatmap_dendrogram_grob <- fig4_pheatmap[["gtable"]][["grobs"]][[1]]
fig4_heatmap_dendrogram_grob[["gp"]] <- gpar(lwd = 0.75)
```

```{r fig4-heatmap-enrichment-plot}
...
fig4_heatmap_pathways_legend_width_pt <- 50

# Generate heatmap of enrichment significance
fig4_heatmap_pathways <- ggplot(df_pathways_heatmap, 
                                aes(x = factor(k), 
                                    y = fct_rev(pathway), 
                                    fill = intensity)) + 
  ...
  scale_fill_gradientn(colors = heatmap_palette,
                       ...
                       guide = guide_colourbar(title.position = "top",
                                               title.hjust = 0.5, 
                                               barwidth = unit(fig4_heatmap_pathways_legend_width_pt, "bigpts"),
...
# Extract legend grob
fig4_heatmap_pathways_legend_grob <- fig4_heatmap_pathways %>%
  ggplotGrob() %>%
  grid.force() %>%
  getGrob("guides.3-3-3-3")

# Remove legend from ggplot object
fig4_heatmap_pathways <- fig4_heatmap_pathways +
  theme(legend.position = "none")
```

```{r fig4-heatmap-human-match-plot}
# Generate heatmap of mouse clusters with human matches
fig4_heatmap_human_match <- ggplot(df_sig_nk,
...
fig4_heatmap_human_match_legend_grob <- fig4_heatmap_human_match %>%
  ggplotGrob() %>%
  grid.force() %>%
  getGrob("guides.3-3-3-3")

# Remove legend from ggplot object
fig4_heatmap_human_match <- fig4_heatmap_human_match +
  theme(legend.position = "none")
```

```{r fig4-heatmap-build}
# Combine heatmaps using patchwork
fig4_heatmap_patchwork <- (fig4_heatmap_human_match / plot_spacer() / fig4_heatmap_pathways) +
  plot_layout(heights = c(0.10, 0.015, 0.930)) &
  theme(plot.margin = margin(l = 0))
fig4_heatmap_patchwork_grob <- patchworkGrob(fig4_heatmap_patchwork)

# Width of dendrogram panel in bigpts
fig4_heatmap_dendrogram_width_pt <- 26

# Dimensions of heatmap patchwork in bigpts
fig4_heatmap_patchwork_width_pt <- fig4_width - fig4_heatmap_dendrogram_width_pt
fig4_heatmap_patchwork_height_pt <- 215

# Heights of padding elements in bigpts
fig4_heatmap_padding_height_1_pt <- 37
fig4_heatmap_padding_height_2_pt <- 19

# Dimensions of heatmap plot
fig4_heatmap_widths <- c(fig4_heatmap_dendrogram_width_pt, fig4_heatmap_patchwork_width_pt)
fig4_heatmap_heights <- c(fig4_heatmap_padding_height_1_pt, 
                          fig4_heatmap_patchwork_height_pt,
                          fig4_heatmap_padding_height_2_pt)

# Layout of heatmap plot
fig4_heatmap_layout <- rbind(c(01, 02),
                             c(03, 02),
                             c(04, 02))

# Grob grid for heatmap plot
fig4_heatmap_grob <- arrangeGrob(zeroGrob(),
                                 fig4_heatmap_patchwork_grob,
                                 fig4_heatmap_dendrogram_grob,
                                 zeroGrob(),
                                 layout_matrix = fig4_heatmap_layout,
                                 widths = unit(fig4_heatmap_widths, "bigpts"),
                                 heights = unit(fig4_heatmap_heights, "bigpts"))

# Dimensions of plot in bigpts
fig4_heatmap_width_pt <- sum(fig4_heatmap_widths)
fig4_heatmap_height_pt <- sum(fig4_heatmap_heights)

# Export plot
outfile <- paste(output_plot_prefix, "heatmap", sep = "_")
outfile <- paste0(outfile, ".pdf")
outfile <- file.path(output_dir, outfile)
export_pdf(x = fig4_heatmap_grob,
           file = outfile,
           width = fig4_heatmap_width_pt,
           height = fig4_heatmap_height_pt,
           units = "bigpts")
```
...
```{r fig4-pathways-cluster-subset}
...
```{r fig4-pathways-cluster-streams}
...
for (i in 1:length(nk_subset)) {
  ...
  for (panel in polar_plot_panel_ids) {
    ...
    polar_plot_nk_labels_x <- polar_plot_nk_grob[["children"]][["panel.7-5-7-5"]][["children"]][[6]][["children"]][[1]][["children"]][[1]][["x"]]
    polar_plot_nk_labels_y <- polar_plot_nk_grob[["children"]][["panel.7-5-7-5"]][["children"]][[6]][["children"]][[1]][["children"]][[1]][["y"]]
    ...
    polar_plot_nk_grob[["children"]][["panel.7-5-7-5"]][["children"]][[6]][["children"]][[1]][["children"]][[1]][["x"]] <- polar_plot_nk_labels_x
    polar_plot_nk_grob[["children"]][["panel.7-5-7-5"]][["children"]][[6]][["children"]][[1]][["children"]][[1]][["y"]] <- polar_plot_nk_labels_y
  }
...
fig4_neuro_ss_padding_pt <- 3

# Padding between panels
fig4_neuro_panel_padding_pt <- 4

# Total padding width 
fig4_ss_padding_width_pt <- fig4_neuro_panel_padding_pt*(nss-1) + fig4_neuro_ss_padding_pt*((nslc_tot - 1) - (nss-1))

# Total width available to place slices
fig4_ss_width_pt <- fig4_width - fig4_ss_padding_width_pt

# Dimensions of mouse slices in bigpts
fig4_mouse_ss_width_pt <- fig4_ss_width_pt/nslc_tot
fig4_mouse_ss_height_pt <- fig4_mouse_ss_width_pt*(mouse_height_px/mouse_width_px)

# Dimensions of human slices in bigpts
fig4_human_ss_width_pt <- fig4_mouse_ss_width_pt
fig4_human_ss_height_pt <- fig4_human_ss_width_pt*(human_height_px/human_width_px)

# Iterate over nk selection
fig4_neuro_ss_grobs <- vector(mode = "list", length = length(list_mouse_slices))
for (i in 1:length(list_mouse_slices)) {
  ...
  fig4_neuro_ss_grobs[[i]] <- vector(mode = "list", length = nslc)
  for (j in 1:nslc) {
    
    # Dimensions of slice series grid
    fig4_neuro_ss_widths <- fig4_mouse_ss_width_pt
    fig4_neuro_ss_heights <- c(fig4_human_ss_height_pt, fig4_neuro_ss_padding_pt, fig4_mouse_ss_height_pt, fig4_neuro_ss_padding_pt, fig4_human_ss_height_pt)
    ...
    ss <- arrangeGrob(list_human_slices_POND[[i]][[j]],
                      ...
                      widths = unit(fig4_neuro_ss_widths, "bigpts"),
                      heights = unit(fig4_neuro_ss_heights, "bigpts"))
    ...
    export_pdf(x = ss,
               file = outfile,
               width = sum(fig4_neuro_ss_widths),
               height = sum(fig4_neuro_ss_heights),
               units = "bigpts")
    
    # Store slice series in list
    fig4_neuro_ss_grobs[[i]][[j]] <- gTree(children = gList(ss, ss_border))
    
  }
}
```

```{r fig4-neuro-panel-grobs}
# Height of polar plot based on ss width
fig4_polar_height_pt <- 2*fig4_mouse_ss_width_pt

# Iterate over neuro panel ss grobs
fig4_neuro_panel_grobs <- vector(mode = "list", length = length(fig4_neuro_ss_grobs))
fig4_neuro_widths_nopadding <- numeric(length(fig4_neuro_ss_grobs))
for (i in 1:length(fig4_neuro_panel_grobs)) {
  
  # Number of slices in the current panel
  nslc <- length(fig4_neuro_ss_grobs[[i]])
  ...
  fig4_neuro_panel_widths <- numeric(ncols)
  fig4_neuro_panel_widths[ind_ss] <- fig4_mouse_ss_width_pt
  fig4_neuro_panel_widths[ind_padding] <- fig4_neuro_ss_padding_pt
  
  # Heights of the panel grids
  fig4_neuro_panel_heights <- c(fig4_polar_height_pt, fig4_neuro_ss_padding_pt, sum(fig4_neuro_ss_heights))
  
  # Build the list of grobs for the panel
  fig4_neuro_panel_grobs_list <- vector(mode = "list", length = ngrobs)
  fig4_neuro_panel_grobs_list[1] <- list_polar_plots[i]
  fig4_neuro_panel_grobs_list[2] <- list(zeroGrob())
  fig4_neuro_panel_grobs_list[ind_ss+2] <- fig4_neuro_ss_grobs[[i]]
  fig4_neuro_panel_grobs_list[ind_padding+2] <- list(zeroGrob())
  
  # Layout of the panel grid
  fig4_neuro_panel_layout <-  rbind(rep(1, ncols),
                                    rep(2, ncols),
                                    3:(ncols+2))
  
  # Grob for the current panel
  fig4_neuro_panel_grobs[[i]] <- arrangeGrob(grobs = fig4_neuro_panel_grobs_list,
                                             layout_matrix = fig4_neuro_panel_layout,
                                             widths = unit(fig4_neuro_panel_widths, "bigpts"),
                                             heights = unit(fig4_neuro_panel_heights, "bigpts"))
  
  # Dimensions of the panel in bigpts
  fig4_neuro_panel_width <- sum(fig4_neuro_panel_widths)
  fig4_neuro_panel_height <- sum(fig4_neuro_panel_heights)  
  
  # Export panel
  outfile <- paste(output_plot_prefix, "neuro_panel_nk", nk_subset[i], sep = "_")
  outfile <- paste0(outfile, ".pdf")
  outfile <- file.path(output_dir, outfile)
  export_pdf(x = fig4_neuro_panel_grobs[[i]],
             file = outfile,
             width = fig4_neuro_panel_width,
             height = fig4_neuro_panel_height,
             units = "bigpts")  
  
  fig4_neuro_widths_nopadding[i] <- fig4_neuro_panel_width
...
fig4_neuro_widths <- numeric(ngrobs)
fig4_neuro_widths[ind_panels] <- fig4_neuro_widths_nopadding
fig4_neuro_widths[ind_padding] <- fig4_neuro_panel_padding_pt

# Heighs for neuro panel
fig4_neuro_heights <- fig4_neuro_panel_height

# Grobs in neuro panel
fig4_neuro_grobs_list <- vector(mode = "list", length = ngrobs)
fig4_neuro_grobs_list[ind_panels] <- fig4_neuro_panel_grobs
fig4_neuro_grobs_list[ind_padding] <- list(zeroGrob())

# Layout for neuro panel grid
fig4_neuro_layout <- rbind(1:ngrobs)

# Grid grob for neuro panel
fig4_neuro_grob <- arrangeGrob(grobs = fig4_neuro_grobs_list,
                               layout_matrix = fig4_neuro_layout,
                               widths = unit(fig4_neuro_widths, "bigpts"),
                               heights = unit(fig4_neuro_heights, "bigpts"))

# Dimensions for neuro panel in bigpts
fig4_neuro_width_pt <- sum(fig4_neuro_widths)
fig4_neuro_height_pt <- sum(fig4_neuro_heights)

# Export neuro panel
outfile <- paste(output_plot_prefix, "neuro_panel", sep = "_")
outfile <- paste0(outfile, ".pdf")
outfile <- file.path(output_dir, outfile)
export_pdf(x = fig4_neuro_grob,
           file = outfile,
           width = fig4_neuro_width_pt,
           height = fig4_neuro_height_pt,
           units = "bigpts")
```

```{r fig4-build}
# Padding between heatmap and neuro panel
fig4_padding_height_pt <- 30

# Figure 3 dimensions in bigpts
fig4_widths <- c(fig4_border_padding_width, 
                 fig4_width, 
                 fig4_border_padding_width)
fig4_heights <- c(fig4_border_padding_height, 
                  fig4_neuro_height_pt,
                  fig4_padding_height_pt,
                  fig4_heatmap_height_pt, 
                  fig4_border_padding_height)

# Figure 3 layout
fig4_layout <- rbind(c(NA, NA, NA),
                     c(NA, 01, NA),
                     c(NA, 02, NA),
                     c(NA, 03, NA),
                     c(NA, NA, NA))
...
# Figure 3 grob
fig4_grob <- arrangeGrob(fig4_neuro_grob,
                         zeroGrob(),
                         fig4_heatmap_grob,
                         layout_matrix = fig4_layout,
                         widths = unit(fig4_widths, "bigpts"),
                         heights = unit(fig4_heights, "bigpts"))

# Figure 3 dimensions in bigpts
fig4_width_pt <- sum(fig4_widths)
fig4_height_pt <- sum(fig4_heights)

# Viewport for pathways heatmap legend
fig4_heatmap_pathways_legend_vp <- viewport(x = unit(fig4_width_pt-8, "bigpts"),
                                            y = unit(25, "bigpts"),
                                            width = unit(fig4_heatmap_pathways_legend_width_pt, "bigpts"),
                                            height = unit(22, "bigpts"),
                                            just = c("right", "bottom"))

# Viewport for human match heatmap legend
fig4_heatmap_human_match_legend_vp_width <- 105
fig4_heatmap_human_match_legend_vp_x <- fig4_width_pt - fig4_heatmap_human_match_legend_vp_width
fig4_heatmap_human_match_legend_vp <- viewport(x = unit(fig4_heatmap_human_match_legend_vp_x, "bigpts"),
                                               y = unit(fig4_heatmap_height_pt+4, "bigpts"),
                                               width = unit(fig4_heatmap_human_match_legend_vp_width, "bigpts"),
                                               height = unit(27, "bigpts"),
                                               just = c("left", "top"))

# Figure 3 dimensions in inches
fig4_width_in <- fig4_width_pt/pt_per_in
fig4_height_in <- fig4_height_pt/pt_per_in

fig4_width_in <- unit(fig4_width_in, "in")
fig4_height_in <- unit(fig4_height_in, "in")
...
pdf(file = outfile,
    width = fig4_width_in,
    height = fig4_height_in)
grid.draw(fig4_grob)
pushViewport(fig4_heatmap_pathways_legend_vp)
grid.draw(fig4_heatmap_pathways_legend_grob)
popViewport()
pushViewport(fig4_heatmap_human_match_legend_vp)
grid.draw(fig4_heatmap_human_match_legend_grob)
popViewport()
dev.off()
```
...
```{r fig4-supp-matching-clusters, eval = FALSE}
outfile <- "fig4_matching_clusters.csv"
outfile <- file.path(output_dir, outfile)
df_sim_pvals %>% 
...
```{r fig4-supp-heatmap-norm-scree-plot, eval = FALSE}
...
```{r fig4-supp-heatmap-norm-build, eval = FALSE}
...
# Generate clustered heatmap
fig4_supp_pheatmap <- pheatmap(mat = mat_pathways_nk_gt4_subset,
                               cluster_col = TRUE,
                               cluster_rows = TRUE,
                               clustering_distance_rows = "euclidean",
                               cutree_rows = hclust_kcut,
...
# Extract dendrograms
fig4_supp_dendrogram_cols <- fig4_supp_pheatmap[["gtable"]][["grobs"]][[1]]
fig4_supp_dendrogram_rows <- fig4_supp_pheatmap[["gtable"]][["grobs"]][[2]]
...
fig4_supp_heatmap_human_match <- ggplot(df_sig_nk_2,
...
fig4_supp_heatmap_clustered <- ggplot(df_pathways_heatmap_2, 
                                      aes(x = cluster_id, 
                                          y = fct_rev(pathway), 
                                          fill = intensity)) + 
  geom_tile(col = "grey60") +
  ...
  theme(strip.background = element_blank(),
        strip.text = element_blank())
fig4_supp_heatmap_clustered_grob <- grid.force(ggplotGrob(fig4_supp_heatmap_clustered))


# Combine heatmaps using patchwork ###########################################
fig4_supp_heatmap_patchwork <- (fig4_supp_heatmap_human_match / fig4_supp_heatmap_clustered) +
  plot_layout(heights = c(0.05, 0.95), guides = "collect") &
  theme(plot.margin = margin(l = 0))
fig4_supp_heatmap_patchwork_grob <- patchworkGrob(fig4_supp_heatmap_patchwork)


# Build supplementary figure #################################################

fig4_supp_heatmap_grob <- arrangeGrob(zeroGrob(),
                                      fig4_supp_dendrogram_cols,
                                      zeroGrob(),
                                      zeroGrob(),
                                      fig4_supp_heatmap_patchwork_grob,
                                      fig4_supp_dendrogram_rows, 
                                      zeroGrob(),
...
outfile <- paste0(outfile, ".pdf")
outfile <- file.path(output_dir, outfile)
export_pdf(x = fig4_supp_heatmap_grob,
           file = outfile,
           width = 20,
           height = 10)
```

```{r fig4-supp-heatmap-scree-plot, eval = FALSE}
mat_pathways_nk_gt4_subset <- df_pathways_nk_gt4_subset %>% 
...
```{r fig4-supp-heatmap-build, eval = FALSE}
# Run hierarchical clustering ################################################
...
```{r fig4-supp-heatmap-no-thresh, eval = FALSE}
# Run hierarchical clustering ################################################
