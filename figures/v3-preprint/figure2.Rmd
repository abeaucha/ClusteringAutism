...
if (run) {
  for (j in jacobians) {
    ...
    indir <- mouse_centroid_dirs[[j]]
    outdir <- mouse_centroid_dirs_50um[[j]]
...
```{r fig2-sim-import}
# Path to mouse-human similarity directory and file
...
df_similarity_nk <- df_similarity_nk %>% 
  mutate(qval = p.adjust(pval, method = "fdr"),
         ...
         pval_lab = case_when(pval >= 0.15 ~ "",
                              pval > 0 & pval < 0.15 ~ paste0("p = ", as.character(round(pval, 3))),
                              pval == 0 ~ "p < 0.001"))
```
...
human_registration_dir <- file.path(PROJECTPATH, "data/human/registration/")
...
dx_palette <- colorRampPalette(c("grey10", "grey80"))(4)
dx_palette <- paste0("seashell", 1:4)
# dx_palette <- paste0("mistyrose", 1:4)
# dx_palette <- paste0("lavenderblush", 1:4)
...
fig2_dx <- ggplot(df_dx_nk_freq,
                  aes(x = cluster_id, y = prop_per_k_per_dx,
                      ...
                      # col = DX_new, 
                      alpha = DX_new)) + 
  geom_col(position = "dodge", alpha = 1.0, col = "grey45", size = 0.35) +
  ...
  labs(x = "Human clusters",
       y = "Proportion of each diagnosis per cluster",
       fill = "Diagnosis",
       col = "Diagnosis") + 
  theme_bw() +
...
```{r fig2-dx-chi2-1}
# Data frame for chi2 test
df_dx_chi2 <- df_dx_nk %>% 
  select(k, DX_new) %>% 
  mutate(k = factor(k),
         DX_new = factor(DX_new))

chisq.test(df_dx_chi2$k,
           df_dx_chi2$DX_new,
...
```{r fig2-dx-chi2-2}
# Remove "Other" DX category
df_dx_chi2 <- df_dx_chi2 %>% 
  filter(DX_new != "Other")
...
pathways_human_palette <- pathways_mouse_palette[c(1, 4)]

# Transparency for human pathway plot
pathway_human_alpha <- pathways_mouse_alpha[c(1, 4)]
