...
```{r fig4-PONDSK-MICe-nk8-heatmap}
pipeline_dir <- file.path(PROJECTPATH, "data/cross_species/v3/375/")
...
# Heatmap plot
fig4_heatmap_PONDSK_MICe_nk8 <- ggplot(df_similarity_nk, 
                                       aes(x = img2_cluster_id, 
                                           y = fct_rev(img1_cluster_id), 
                                           fill = similarity)) + 
  geom_tile(col = "grey50") + 
...
# Export
outfile <- paste0("figure3_PONDSK_MICe_heatmap_nk8.pdf")
outfile <- file.path(output_dir, outfile)
pdf(file = outfile,
    width = unit(500/72, "in"),
    height = unit(500/72, "in"))
print(fig4_heatmap_PONDSK_MICe_nk8)
dev.off()
```

```{r}
df_matches_PONDSK_MICe_nk8 <- df_similarity_nk %>% 
  # filter(significant) %>% 
  filter(pval_lab != "") %>% 
  select(POND = img1_cluster_id, MICe = img2_cluster_id)
```
...
# Heatmap plot
fig4_heatmap_HBN_MICe_nk8 <- ggplot(df_similarity_nk, 
                                    aes(x = img2_cluster_id, 
                                        y = fct_rev(img1_cluster_id), 
                                        fill = similarity)) + 
  geom_tile(col = "grey50") + 
...
# Export
outfile <- paste0("figure3_HBN_MICe_heatmap_nk8.pdf")
outfile <- file.path(output_dir, outfile)
pdf(file = outfile,
    width = unit(500/72, "in"),
    height = unit(500/72, "in"))
print(fig4_heatmap_HBN_MICe_nk8)
dev.off()
```


```{r}
df_matches_HBN_MICe_nk8 <- df_similarity_nk %>% 
  # filter(significant) %>% 
  filter(pval_lab != "") %>% 
  select(HBN = img1_cluster_id, MICe = img2_cluster_id)
```
...
df_similarity_nk <- df_similarity_nk %>% 
  mutate(qval = p.adjust(pval, method = "fdr"),
         ...
         pval_lab = case_when(pval > 0.10 ~ "",
                              pval >= 0.001 & pval <= 0.10 ~ paste0("p = ", as.character(round(pval, 3))),
                              pval < 0.001 ~ "p < 0.001"),
         pval_lab = ifelse(img1_cluster_id == "8-1" & img2_cluster_id == "8-8", paste0("p = ", as.character(round(pval, 3))) ,pval_lab),
         pval_lab = ifelse(img1_cluster_id == "8-4" & img2_cluster_id == "8-4", paste0("p = ", as.character(round(pval, 3))), pval_lab),
         pval_lab = ifelse(img1_cluster_id == "8-4" & img2_cluster_id == "8-5", paste0("p = ", as.character(round(pval, 3))), pval_lab),
         pval_lab = ifelse(img1_cluster_id == "8-7" & img2_cluster_id == "8-3", paste0("p = ", as.character(round(pval, 3))), pval_lab),
         cor_lab = ifelse(pval_lab == "", "", paste0("r = ", as.character(round(similarity, 3)))),
         stat_lab = ifelse(pval_lab == "", "", paste(pval_lab, cor_lab, sep = "\n")))
...
# Heatmap plot
fig4_heatmap_PONDSK_HBN_nk8 <- ggplot(df_similarity_nk, 
                                      aes(x = img2_cluster_id, 
                                          y = fct_rev(img1_cluster_id), 
                                          fill = similarity)) + 
  geom_tile(col = "grey50") + 
...
# Export
outfile <- paste0("figure3_PONDSK_HBN_heatmap_nk8.pdf")
outfile <- file.path(output_dir, outfile)
pdf(file = outfile,
    width = unit(500/72, "in"),
    height = unit(500/72, "in"))
print(fig4_heatmap_PONDSK_HBN_nk8)
dev.off()
```
```{r}
df_similarity_nk %>% 
  filter(img1_cluster_id == "8-2")
```


```{r}
df_similarity_nk %>% 
  filter(img1_cluster_id == "8-1")
```

```{r}
df_similarity_nk %>% 
  filter(img1_cluster_id == "8-4")
```

```{r}
df_similarity_nk %>% 
  filter(img1_cluster_id == "8-7")
```

```{r}
df_matches_PONDSK_HBN_nk8 <- df_similarity_nk %>% 
  # filter(significant) %>% 
  filter(pval_lab != "") %>% 
  select(POND = img1_cluster_id, HBN = img2_cluster_id)
```
...
fig4_heatmap <- (fig4_heatmap_PONDSK_MICe | fig4_heatmap_HBN_MICe | fig4_heatmap_PONDSK_HBN) /
  (fig4_heatmap_PONDSK_MICe_nk8 | fig4_heatmap_HBN_MICe_nk8 | fig4_heatmap_PONDSK_HBN_nk8) +
...
outfile <- paste0("figure3_heatmap_nk8.pdf")
outfile <- file.path(output_dir, outfile)
pdf(file = outfile,
