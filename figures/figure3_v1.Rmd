...
# Dictionary of pathway terms and labels
df_pathway_labels <- tibble(pathway = pathway_terms,
...
df_mouse_pathways_heatmap <- df_mouse_pathways_nk_gt4 %>% 
  ...
  mutate(pathway = factor(pathway, levels = pathway_lvls_clustered),
         label = factor(label, levels = pathway_label_lvls_clustered))
...
fig3_mouse_pathways_heatmap_grob <- grid.force(ggplotGrob(fig3_mouse_pathways_heatmap))

fig3_mouse_pathways_dendrogram_grob <- pheatmap_out[["gtable"]][["grobs"]][[1]]
fig3_mouse_pathways_dendrogram_grob[["gp"]] <- gpar(lwd = 0.75)

fig3_mouse_pathways_layout <- rbind(c(01, 02),
                                    c(03, 02),
                                    c(04, 02))

fig3_mouse_pathways_widths <- c(0.05, 0.95)
fig3_mouse_pathways_heights <- c(0.025, 0.875, 0.1)
fig3_mouse_pathways_heights <- c(7.5, 96, 18.5)

fig3_mouse_pathways_grob <- arrangeGrob(zeroGrob(), 
                                        fig3_mouse_pathways_heatmap_grob,
                                        fig3_mouse_pathways_dendrogram_grob,
                                        zeroGrob(),
                                        layout_matrix = fig3_mouse_pathways_layout,
                                        widths = unit(fig3_mouse_pathways_widths, "npc"),
                                        heights = unit(fig3_mouse_pathways_heights, "bigpts"))

fig3_mouse_pathways_width_pt <- 510
fig3_mouse_pathways_height_pt <- sum(fig3_mouse_pathways_heights)

fig3_mouse_pathways_width_in <- fig3_mouse_pathways_width_pt/pt_per_in
fig3_mouse_pathways_height_in <- fig3_mouse_pathways_height_pt/pt_per_in

fig3_mouse_pathways_width_in <- unit(fig3_mouse_pathways_width_in, "in")
fig3_mouse_pathways_height_in <- unit(fig3_mouse_pathways_height_in, "in")

outfile <- paste(output_plot_prefix, "mouse_pathways", sep = "_")
outfile <- paste0(outfile, ".pdf")
outfile <- file.path(output_dir, outfile)

pdf(file = outfile,
    width = fig3_mouse_pathways_width_in,
    height = fig3_mouse_pathways_height_in)
grid.draw(fig3_mouse_pathways_grob)
dev.off()
```
```{r}
tmp <- fig3_mouse_pathways_dendrogram_grob
...
```{r}
grid.ls(fig3_mouse_pathways_heatmap_grob)
```
...
```{r}
tmp_layout <- rbind(c(1, 2))

tmp_widths <- c(0.2, 0.8)

tmp <- arrangeGrob(fig3_mouse_pathways_dendrogram_grob,
                   fig3_mouse_pathways_heatmap_grob,
                   layout_matrix = tmp_layout,
                   widths = tmp_widths)

tmp_width_pt <- 510
tmp_height_pt <- 240

tmp_width_in <- tmp_width_pt/pt_per_in
tmp_height_in <- tmp_height_pt/pt_per_in

tmp_width_in <- unit(tmp_width_in, "in")
tmp_height_in <- unit(tmp_height_in, "in")

outfile <- paste(output_plot_prefix, "tmp", sep = "_")
outfile <- paste0(outfile, ".pdf")
outfile <- file.path(output_dir, outfile)

pdf(file = outfile,
    width = tmp_width_in,
    height = tmp_height_in)
grid.draw(tmp)
dev.off()
```
