...
for (i in 1:nrow(df_mouse_matches)) {
  ...
  ss <- sliceSeries(nrow = 1, ncol = 1, dimension = mouse_slc_dim, slices = mouse_slc) %>% 
    anatomy(mouse_anat_vol, low = 700, high = 1400) %>% 
    overlay(img, low = overlay_threshold[1], high = overlay_threshold[2], 
            symmetric = TRUE) 
  
  # Slice dimensions in pixels
  width_px <- dim(mouse_anat_vol)[2]
  height_px <- dim(mouse_anat_vol)[3]
...
for (i in 1:nrow(df_human_matches)) {
  ...
  # Generate slice series
  ss <- sliceSeries(nrow = 1, ncol = 1, dimension = human_slc_dim, slices = human_slc) %>% 
  ...
  # Slice dimensions in pixels
  width_px <- dim(human_anat_vol)[2]
  height_px <- dim(human_anat_vol)[3]
