...
if (pathway_stat == "qvals") {
  ...
  df_pathways_heatmap <- df_pathways_heatmap %>% 
    mutate(intensity = ifelse(NLQ > stat_threshold, stat_threshold, NLQ))
  heatmap_limits <- c(2, stat_threshold)
  heatmap_fill_lab <- "-log10(q)"
} else if (pathway_stat == "enrichment") {
  stat_threshold <- 45
  df_pathways_heatmap <- df_pathways_heatmap %>% 
    mutate(intensity = ifelse(E > stat_threshold, stat_threshold, E))
  heatmap_limits <- c(0, stat_threshold)
  heatmap_fill_lab <- "Enrichment"
} else {
  stop()
}
...
# Panel: Mouse and human slice series
...
# Iterate over mouse clusters
list_mouse_slices <- vector(mode = "list", length = nrow(df_mouse_matches))
for (i in 1:nrow(df_mouse_matches)) {
  
  # Extract cluster info
  nk <- df_mouse_matches[[i, "nk"]]
  k <- df_mouse_matches[[i, "k"]]
  ...
  # # Slice dimensions in pixels
  # mouse_width_px <- dim(mouse_anat_vol)[2]
  # mouse_height_px <- dim(mouse_anat_vol)[3]
  # 
  # # Image dimensions
  # mouse_height_pt <- 15
  # mouse_width_pt <- mouse_height_pt*(mouse_width_px/mouse_height_px)
  # 
  # mouse_width_in <- mouse_width_pt/pt_per_in
  # mouse_height_in <- mouse_height_pt/pt_per_in
  # 
  # mouse_width_in <- unit(mouse_width_in, "in")
  # mouse_height_in <- unit(mouse_height_in, "in")
  # 
  # outfile <- paste(output_plot_prefix, "mouse_cluster_centroid_nk",
  #                  nk, "k", k, sep = "_")
  # outfile <- paste0(outfile, ".pdf")
  # outfile <- file.path(output_dir, outfile)
  # pdf(file = outfile,
  #     width = mouse_width_in,
  #     height = mouse_height_in)
  # draw(ss)
  # dev.off()
...
```{r mouse-ss-grob}
list_mouse_ss_grobs <- vector(mode = "list", length = 2*length(list_mouse_slices))
jseq <- seq(1, 2*length(list_mouse_slices), by = 2)
for (i in 1:length(list_mouse_slices)) {
  list_mouse_ss_grobs[[jseq[i]]] <- list_mouse_slices[[i]]
  list_mouse_ss_grobs[[jseq[i]+1]] <- zeroGrob()
}
...
# Number of slices
nss <- length(list_mouse_slices)

# Width of padding between slices
padding_width_pt <- 2

# Width of padding between clusters
padding_big_width_pt <- 2*padding_width_pt

# Set padding widths in grid
fig3_mouse_ss_widths <- rep(c(0, padding_width_pt), nss)
fig3_mouse_ss_widths[c(4*1:3, 6*3:6)] <- padding_big_width_pt

# Total width of padding
padding_width_tot <- sum(fig3_mouse_ss_widths)

# Total width of slices
ss_width_tot <- fig3_width - padding_width_tot

# Figure dimensions of individual slices
mouse_width_pt <- ss_width_tot/nss
mouse_height_pt <- mouse_width_pt*(mouse_height_px/mouse_width_px)

# Figure grid widths and heights
fig3_mouse_ss_widths[2*(1:nss)-1] <- mouse_width_pt
fig3_mouse_ss_heights <- mouse_height_pt

# Figure grid layout
fig3_mouse_ss_layout <- rbind(1:length(list_mouse_ss_grobs))

# Figure grob
fig3_mouse_ss_grob <- arrangeGrob(grobs = list_mouse_ss_grobs,
                                  layout_matrix = fig3_mouse_ss_layout,
                                  widths = unit(fig3_mouse_ss_widths, "bigpts"),
                                  heights = unit(fig3_mouse_ss_heights, "bigpts"))

fig3_mouse_ss_width_pt <- sum(fig3_mouse_ss_widths)
fig3_mouse_ss_height_pt <- sum(fig3_mouse_ss_heights)

fig3_mouse_ss_width_in <- fig3_mouse_ss_width_pt/pt_per_in
fig3_mouse_ss_height_in <- fig3_mouse_ss_height_pt/pt_per_in

fig3_mouse_ss_width_in <- unit(fig3_mouse_ss_width_in, "in")
fig3_mouse_ss_height_in <- unit(fig3_mouse_ss_height_in, "in")

outfile <- paste(output_plot_prefix, "ss_mouse", sep = "_")
outfile <- paste0(outfile, ".pdf")
outfile <- file.path(output_dir, outfile)
pdf(file = outfile,
    width = fig3_mouse_ss_width_in,
    height = fig3_mouse_ss_height_in)
grid.draw(fig3_mouse_ss_grob)
dev.off()
```
...
# Determine human clusters matching mouse clusters
list_human_matches <- vector(mode = "list", length = nrow(df_mouse_matches))
for (i in 1:nrow(df_mouse_matches)) {
  
  # Extract matches to mouse cluster
  df_match_i <- df_sim_pvals %>% 
    filter(mouse_nk == df_mouse_matches[[i, "nk"]],
           mouse_k == df_mouse_matches[[i, "k"]],
           significant == 1)
  ...
  list_human_matches[[i]] <- df_match_i
  
}

# Reduce list to a data frame
df_human_matches <- bind_rows(list_human_matches)

# Iterate over human clusters
list_human_slices <- vector(mode = "list", length = nrow(df_human_matches))
for (i in 1:nrow(df_human_matches)) {
  
  # Extract cluster info
  nk <- df_human_matches[[i, "nk"]]
  k <- df_human_matches[[i, "k"]]
  ...
  # # Slice dimensions in pixels
  # human_width_px <- dim(human_anat_vol)[2]
  # human_height_px <- dim(human_anat_vol)[3]
  # 
  # # Image dimensions
  # human_height_pt <- mouse_height_pt
  # human_width_pt <- human_height_pt*(human_width_px/human_height_px)
  # 
  # human_height_in <- human_height_pt/pt_per_in
  # human_width_in <- human_width_pt/pt_per_in
  # 
  # human_width_in <- unit(human_width_in, "in")
  # human_height_in <- unit(human_height_in, "in")
  # 
  # outfile <- paste(output_plot_prefix, "human_cluster_centroid_nk",
  #                  nk, "k", k, sep = "_")
  # outfile <- paste0(outfile, ".pdf")
  # outfile <- file.path(output_dir, outfile)
  # pdf(file = outfile,
  #     width = human_width_in,
  #     height = human_height_in)
  # draw(ss)
  # dev.off()
...
```{r human-ss-grob}
list_human_ss_grobs <- vector(mode = "list", length = 2*length(list_human_slices))
jseq <- seq(1, 2*length(list_human_slices), by = 2)
for (i in 1:length(list_human_slices)) {
  list_human_ss_grobs[[jseq[i]]] <- list_human_slices[[i]]
  list_human_ss_grobs[[jseq[i]+1]] <- zeroGrob()
}
...
# Image dimensions
human_width_pt <- mouse_width_pt
human_height_pt <- human_width_pt*(human_height_px/human_width_px)

# Figure grid widths and heights
fig3_human_ss_widths <- fig3_mouse_ss_widths
fig3_human_ss_heights <- human_height_pt

# Figure grid layout
fig3_human_ss_layout <- fig3_mouse_ss_layout

# Figure grob
fig3_human_ss_grob <- arrangeGrob(grobs = list_human_ss_grobs,
                                  layout_matrix = fig3_human_ss_layout,
                                  widths = unit(fig3_human_ss_widths, "bigpts"),
                                  heights = unit(fig3_human_ss_heights, "bigpts"))

# Dimensions in bigpts
fig3_human_ss_width_pt <- sum(fig3_human_ss_widths)
fig3_human_ss_height_pt <- sum(fig3_human_ss_heights)

# Dimensions in inches
fig3_human_ss_width_in <- fig3_human_ss_width_pt/pt_per_in
fig3_human_ss_height_in <- fig3_human_ss_height_pt/pt_per_in

fig3_human_ss_width_in <- unit(fig3_human_ss_width_in, "in")
fig3_human_ss_height_in <- unit(fig3_human_ss_height_in, "in")

# Export
outfile <- paste(output_plot_prefix, "ss_human", sep = "_")
outfile <- paste0(outfile, ".pdf")
outfile <- file.path(output_dir, outfile)
pdf(file = outfile,
    width = fig3_human_ss_width_in,
    height = fig3_human_ss_height_in)
grid.draw(fig3_human_ss_grob)
dev.off()
```
...
```{r fig3-ss}
fig3_ss_widths <- fig3_width
fig3_ss_heights <- c(fig3_human_ss_heights, fig3_mouse_ss_heights)

fig3_ss_layout <- cbind(1:2)

fig3_ss_grob <- arrangeGrob(fig3_human_ss_grob,
                            fig3_mouse_ss_grob,
                            layout_matrix = fig3_ss_layout,
                            widths = unit(fig3_width, "bigpts"),
                            heights = unit(fig3_ss_heights, "bigpts"))

# Dimensions in bigpts
fig3_ss_width_pt <- sum(fig3_ss_widths)
fig3_ss_height_pt <- sum(fig3_ss_heights)

# Dimensions in inches
fig3_ss_width_in <- fig3_ss_width_pt/pt_per_in
fig3_ss_height_in <- fig3_ss_height_pt/pt_per_in

fig3_ss_width_in <- unit(fig3_ss_width_in, "in")
fig3_ss_height_in <- unit(fig3_ss_height_in, "in")
...
outfile <- file.path(output_dir, outfile)
pdf(file = outfile,
    width = fig3_ss_width_in,
    height = fig3_ss_height_in)
grid.draw(fig3_ss_grob)
dev.off()
```


```{r fig3-matching-clusters}
...
# Panel: Mouse pathway polar plots

```{r}
df_pathways_nk4 <- df_pathways_all %>% 
  filter(nk == 4)

polar_plot_lvls <- c(" ", pathway_lvls_clustered)

# Create a temp df for the additional empty factor level
df_polar_plot_tmp <- df_pathways_nk4 %>% 
...
# Palette for pathway plot
pathways_palette_nk4 <- c(carto_pal(12, "Prism")[2],
                          carto_pal(12, "Antique")[7],
                          carto_pal(12, "Antique")[2],
                          carto_pal(12, "Prism")[8])

# Generate polar plot
fig3_pathways_nk4 <- ggplot(df_polar_plot, 
                            aes(x = pathway, 
                                ymin = 0, ymax = NLQ, 
                                group = cluster_id, 
                                fill = cluster_id, 
  ...
  scale_colour_manual(values = pathways_palette_nk4,
                      guide = guide_legend(title.position = "bottom",
                                           hjust = 0)) + 
  scale_fill_manual(values = pathways_palette_nk4,
                    guide = guide_legend(title.position = "bottom",
                                         hjust = 0)) +
...
# Extract legend grob
fig3_pathways_nk4_legend_grob <- fig3_pathways_nk4 %>% 
  ggplotGrob() %>% 
  grid.force() %>% 
  getGrob("guides.3-3-3-3")

# Remove legend
fig3_pathways_nk4 <- fig3_pathways_nk4 +
  theme(legend.position = "none")

# Plot grob
fig3_pathways_nk4_grob <- grid.force(ggplotGrob(fig3_pathways_nk4))

# Fix polar axis labels
fig3_pathways_nk4_grob <- grid.force(ggplotGrob(fig3_pathways_nk4))
fig3_pathways_nk4_grob[["children"]][["panel.7-5-7-5"]][["children"]][[5]][["children"]][[1]][["children"]][[1]][["label"]] <- pathway_label_lvls_clustered

# fig3_pathways_nk4_grob[["children"]][["panel.7-5-7-5"]][["children"]][[5]][["children"]][[1]][["children"]][[1]][["x"]] <- unit(c(0.50, 0.80, 1.00, 1.00, 0.80, 0.50, 0.22, 0.01, 0.01, 0.16), "npc")
# fig3_pathways_nk4_grob[["children"]][["panel.7-5-7-5"]][["children"]][[5]][["children"]][[1]][["children"]][[1]][["y"]] <- unit(c(0.99, 0.91, 0.68, 0.35, 0.09, 0.02, 0.09, 0.35, 0.65, 0.91), "npc")

grid.newpage()
grid.draw(fig3_pathways_nk4_grob)
```

```{r}
df_pathways_nk7 <- df_pathways_all %>% 
  filter(nk == 7)

polar_plot_lvls <- c(" ", pathway_lvls_clustered)

# Create a temp df for the additional empty factor level
df_polar_plot_tmp <- df_pathways_nk7 %>% 
  mutate(pathway = as.character(pathway)) %>% 
  filter(pathway == polar_plot_lvls[length(polar_plot_lvls)]) %>% 
  mutate(pathway = " ")

# Join the empty factor level to the true factors
df_polar_plot <- df_pathways_nk7 %>% 
  bind_rows(df_polar_plot_tmp) %>% 
  mutate(pathway = factor(pathway, levels = polar_plot_lvls))
...
# Palette for pathway plot
# pathways_palette_nk7 <- c(carto_pal(12, "Prism")[2],
#                           carto_pal(12, "Antique")[7],
#                           carto_pal(12, "Antique")[2],
#                           carto_pal(12, "Prism")[8])

# Generate polar plot
fig3_pathways_nk7 <- ggplot(df_polar_plot, 
                            aes(x = pathway, 
                                ymin = 0, ymax = NLQ, 
                                group = cluster_id, 
                                fill = cluster_id, 
  ...
  scale_x_discrete(expand = expansion(), 
                   labels = pathway_label_lvls_clustered) + 
  scale_y_continuous(breaks = polar_breaks) +
  # scale_colour_manual(values = pathways_palette_nk7,
  #                     guide = guide_legend(title.position = "bottom",
  #                                          hjust = 0)) + 
  # scale_fill_manual(values = pathways_palette_nk7,
  #                   guide = guide_legend(title.position = "bottom",
  #                                        hjust = 0)) +
  # scale_size_manual(values = c(0.6, 0.4, 0.4, 0.6)) + 
  # scale_alpha_manual(values = c(0.3, 0.2, 0.2, 0.3)) +
...
# Extract legend grob
fig3_pathways_nk7_legend_grob <- fig3_pathways_nk7 %>% 
  ggplotGrob() %>% 
  grid.force() %>% 
  getGrob("guides.3-3-3-3")

# Remove legend
fig3_pathways_nk7 <- fig3_pathways_nk7 +
  theme(legend.position = "none")

# Plot grob
fig3_pathways_nk7_grob <- grid.force(ggplotGrob(fig3_pathways_nk7))

# Fix polar axis labels
fig3_pathways_nk7_grob <- grid.force(ggplotGrob(fig3_pathways_nk7))
fig3_pathways_nk7_grob[["children"]][["panel.7-5-7-5"]][["children"]][[5]][["children"]][[1]][["children"]][[1]][["label"]] <- pathway_label_lvls_clustered

# fig3_pathways_nk7_grob[["children"]][["panel.7-5-7-5"]][["children"]][[5]][["children"]][[1]][["children"]][[1]][["x"]] <- unit(c(0.50, 0.80, 1.00, 1.00, 0.80, 0.50, 0.22, 0.01, 0.01, 0.16), "npc")
# fig3_pathways_nk7_grob[["children"]][["panel.7-5-7-5"]][["children"]][[5]][["children"]][[1]][["children"]][[1]][["y"]] <- unit(c(0.99, 0.91, 0.68, 0.35, 0.09, 0.02, 0.09, 0.35, 0.65, 0.91), "npc")

grid.newpage()
grid.draw(fig3_pathways_nk7_grob)
```

```{r}
fig3_polar_layout <- rbind(1:3)

fig3_polar_grob <- arrangeGrob(fig3_pathways_nk4_grob,
                               empty_rect_grob,
                               fig3_pathways_nk7_grob,
                               layout_matrix = fig3_polar_layout)
...
# Create a temp df for the additional empty factor level
df_polar_plot_tmp <- df_pathways_subset %>% 
  mutate(pathway = as.character(pathway)) %>% 
  filter(pathway == polar_plot_lvls[length(polar_plot_lvls)]) %>% 
  mutate(pathway = " ")

# Join the empty factor level to the true factors
df_polar_plot <- df_pathways_subset %>% 
  bind_rows(df_polar_plot_tmp) %>% 
  mutate(pathway = factor(pathway, levels = polar_plot_lvls))
...
fig3_heatmap_layout <- rbind(c(01, 01, 01, 01),
                             c(02, 02, 02, 02),
                             c(03, 04, 05, 06),
                             c(07, 04, 04, 04),
                             c(08, 04, 04, 04))

fig3_heatmap_widths <- fig3_width*c(0.05, 0.75, 0.12, 0.08)
fig3_heatmap_heights <- c(fig3_ss_height_pt, 19, 19, 205, 19)

fig3_heatmap_grob <- arrangeGrob(fig3_ss_grob,
                                 # rect_w_num(2),
                                 zeroGrob(),
                                 # rect_w_num(3),
                                 zeroGrob(),
                                 fig3_heatmap_patchwork_grob,
                                 fig3_heatmap_human_match_legend_grob,
                                 # rect_w_num(6),
                                 zeroGrob(),
...
fig3_heatmap_legend_vp <- viewport(x = unit(fig3_width-10, "bigpts"),
                                   y = unit(0, "bigpts"),
                                   width = unit(50, "bigpts"),
                                   height = unit(45, "bigpts"),
                                   just = c("right", "bottom"))
...
pushViewport(fig3_heatmap_legend_vp)
